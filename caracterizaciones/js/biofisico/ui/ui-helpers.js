...
function resolveTerritoryContext({ mpnombre, dpnombre, deps = {} }) {
    ...
    const normalizedDepartment = normalizeDepartmentCode(currentDepartmentId);
    const normalizedMunicipality = normalizeMunicipalityCode(currentMunicipalityId);
    const departmentFromMunicipality = normalizeDepartmentCode(normalizedMunicipality.substring(0, 2));

    if (territoryLevel === "MUNI" && normalizedMunicipality) {
        const municipality = municipalityNames[normalizedMunicipality] || pickReadableName(mpnombre) || normalizedMunicipality;
        const department =
            departmentNames[departmentFromMunicipality] ||
            departmentNames[normalizedDepartment] ||
            pickReadableName(dpnombre) ||
            "";

        return department ? `${municipality}, ${department}` : municipality;
    }

    if (territoryLevel === "DEPTO" && normalizedDepartment) {
        return departmentNames[normalizedDepartment] || pickReadableName(dpnombre) || "Colombia";
    }

    const municipality = municipalityNames[normalizedMunicipality] || pickReadableName(mpnombre);
    const department =
        departmentNames[departmentFromMunicipality] ||
        departmentNames[normalizedDepartment] ||
        pickReadableName(dpnombre);
...
export function updateChartTitle(config, mpnombre, dpnombre, deps = {}) {
    const titleElement = document.getElementById("chartTitle");
    if (!titleElement) return;

    const titleDpnombre = pickReadableName(dpnombre) || deps.departmentNames?.[normalizeDepartmentCode(deps.currentDepartmentId)] || null;
