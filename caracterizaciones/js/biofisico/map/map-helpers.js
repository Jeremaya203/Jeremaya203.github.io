...
export function getLandformsScaleTitle(scale) {
...
export function getDepartmentBasinsGroupField(config, layer) {
    const map = config?.cuencasAgg?.groupByLayerId || {};
    const layerId = layer?.layerId;
    return map[layerId] || config?.cuencasAgg?.groupField || "zonahid";
}

export function buildBasinsDictionaryFromRenderer(layerJson) {
