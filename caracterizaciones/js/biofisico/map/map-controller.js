...
import { getBiofisicoMapRegistryEntry } from "./map-registry.js?v=clima-stations-cleanup-20260717";
import {
    createBiofisicoFeatureLayer,
    createBiofisicoImageryLayer,
    createBiofisicoVariantLayers,
    configureVariantLayerLabels
} from "./map-layer-factory.js?v=pendientes-imageserver-20260716";
import { slopeColors } from "../config.js?v=pendientes-imageserver-20260716";
import {
    buildFeatureQuerySignature,
    createChartPrefetchQuery,
    createExtentQuery
} from "./map-query-builder.js";
import {
...
export function createMapController({ getState }) {
    ...
    function getRenderKey(config) {
        ...
        return buildActiveLayerRenderKey({
            ...
            currentMunicipalityId: state.currentMunicipalityId,
    ...
    function isSlopesImageryConfig(config) {
        return config?.id === "pendientes" &&
            config?.mapLayerType === "imagery" &&
            Boolean(config?.mapUrl);
    }

    function shouldUseSlopesImagery(deps) {
        return isSlopesImageryConfig(deps.config) &&
            deps.getFiltroNivel?.() === "MUNI" &&
            Boolean(String(deps.getMunicipioActual?.() || "").trim());
    }

    function buildSlopesImageryWhere(deps) {
        const field = deps.config?.mapNameField || "name";
        const municipality = String(deps.getMunicipioActual?.() || "").trim();

        if (deps.getFiltroNivel?.() === "MUNI" && municipality) {
            return `${field} LIKE '%_${municipality.replace(/'/g, "''")}'`;
        }

        return "1=0";
    }

    async function zoomToSlopesCatalogExtent(deps, where) {
    ...
    function buildClimateRiskDepartmentWhere(deps) {
        if (deps.config?.id !== "riesgo_cc" || !deps.getMunicipioActual?.()) {
            return null;
        }

        const municipality = String(deps.getMunicipioActual?.() || "").trim();
        const departmentId = String(deps.getDeptoActual?.() || municipality.substring(0, 2)).trim();
    ...
    function getMapWhereForConfig(deps) {
        if (shouldUseSlopesImagery(deps)) {
            return buildSlopesImageryWhere(deps);
        }
        return buildClimateRiskDepartmentWhere(deps) || deps.getWhereBase() || "1=1";
    }

    function canReuseSingleLayer(deps, config, nextStructureKey) {
        if (shouldUseSlopesImagery(deps)) return false;
    ...
    function renderActiveLayer(deps) {
        const startLayerLoad = getMapNow();
        ...
        if (nextRenderKey === activeLayerRenderKey && hasActiveRenderableLayer()) {
            ...
            if (activeLayer) {
                deps.updateChart(activeLayer, config);
            }
            deps.updateSummary();
            return;
        }
        ...
        if (shouldUseSlopesImagery({ ...deps, config })) {
            renderSlopesImageryLayer({
                ...deps,
                config,
                currentCycle,
                startLayerLoad
    ...
    function renderSlopesImageryLayer(deps) {
        const mapWhere = buildSlopesImageryWhere(deps);
        ...
        deps.updateSource(chartLayer);
        addBiofisicoLayerToMap(deps.map, imageryLayer);
        deps.updateSummary();

        const legendEntries = Object.entries(slopeColors)
        ...
        deps.updateChart(chartLayer, deps.config, { skipSyncMap: true });

        const zoomPromise = zoomToSlopesCatalogExtent(deps, mapWhere)
            .catch(error => {
                const message = String(error?.message || "").toLowerCase();
        ...
        imageryLayer.when(async () => {
            ...
            deps.recordBiofisicoMetric(
                "loadCurrentLayer.pendientesImagery",
                getMapNow() - deps.startLayerLoad,
                {
    ...
    function renderVariantLayers(deps) {
        ...
        const legendTitle = (deps.config.id === "geoformas")
            ? deps.getLandformsScaleTitle(deps.view.scale)
            : deps.config.title;
        ...
        active.when(async () => {
            ...
            if (deps.config.isGeoforma && deps.config.isGeoformaDualChart) {
                try {
                    await deps.ensureLandformDictionary();
                } catch (e) {
            ...
            deps.updateChart(chartLayer, deps.config, {
    ...
    function createVariantScaleWatcher(deps) {
        const onScale = deps.debounce(() => {
            ...
            if (deps.config?.id === "geoformas") {
                const legendTitle = deps.getLandformsScaleTitle?.(deps.view.scale) || deps.config.title;
    ...
    function renderSingleLayer(deps) {
        ...
        deps.updateSource(newLayer);
        const chartBeforeMap = shouldRenderChartBeforeMap(deps);

        if (chartBeforeMap) {
            deps.updateChart(newLayer, deps.config, { skipSyncMap: true });
        }
        ...
        newLayer.when(async () => {
            ...
            void zoomPromise;
            deps.highlightClimateRiskMunicipality?.(newLayer);
            ...
            if (deps.config.isSuelos && deps.config.suelosType === "orden") {
                try {
                    await deps.ensureSoilOrderDictionary(deps.config.url);
                } catch (e) {
            ...
            if (deps.config.isGeoforma && deps.config.isGeoformaDualChart) {
                try {
                    await deps.ensureLandformDictionary(deps.config.url);
                } catch (e) {
                    console.warn("No se pudo cargar dict geoformas:", e);
                }
            }
    ...
    function renderReusedSingleLayer(deps) {
        ...
        deps.updateSummary();
        if (chartBeforeMap) {
            deps.updateChart(layer, deps.config, { skipSyncMap: true });
        }
        ...
        layer.when(async () => {
            ...
            void zoomPromise;
            deps.highlightClimateRiskMunicipality?.(layer);
            revealLayerWhenReady({
                layer,
                config: deps.config,
                zoomPromise,
