export function buildStationsPopupContent(evt, deps) {
    ...
    const { escapeHtml, municipalityNames, departmentNames } = deps;
    ...
    const rawMunicipalityName = String(attributes.mpnombre ?? "").trim();
    const rawDepartmentName = String(attributes.dpnombre ?? "").trim();

    const municipalityName = (!rawMunicipalityName || rawMunicipalityName === mpCode || !isNaN(rawMunicipalityName))
        ? (municipalityNames?.[mpCode] || rawMunicipalityName || mpCode)
        : rawMunicipalityName;

    const departmentName = (!rawDepartmentName || rawDepartmentName === dpCode || !isNaN(rawDepartmentName))
        ? (departmentNames?.[dpCode] || rawDepartmentName || dpCode)
        : rawDepartmentName;

    return `
        <div class="oot-js-biofisico-mappopup-3">
        <div><b>Estacion:</b> ${esc(attributes.nombest)} (${esc(attributes.codest)})</div>
        <div><b>Municipio:</b> ${esc(municipalityName)} (${esc(mpCode)})</div>
        <div><b>Departamento:</b> ${esc(departmentName)} (${esc(dpCode)})</div>
