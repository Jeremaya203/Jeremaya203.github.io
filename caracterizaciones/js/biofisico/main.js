...
} from "./events/biophysical-events.js?v=component-nav-biofisico-20260623";
import {
...
import {
    ...
    clearLegend,
    setSummaryText,
    updateChartTitle as actualizarTituloGraficoRaw
} from "./ui/ui-helpers.js?v=titulos-departamentales-20260617";
import {
    renderBiofisicoControls,
    renderBiofisicoSubTabs
} from "./ui/biophysical-controls.js?v=hipsometria-axis-spacing-20260723";
import {
    renderBiofisicoSummary
} from "./ui/biophysical-summary-renderer.js?v=hipsometria-depto-panel-20260618";
import {
...
import {
    ...
    sqlLiteral,
    sqlStartsWith
} from "./utils/biophysical-format-utils.js";
import {
    buildBasinsDictionaryFromRenderer,
    destroyLayerSafe,
    getDepartmentBasinsGroupField,
    pickLayerByScale,
    getLandformsScaleTitle,
    pickExistingField,
    pickVariantByScale
} from "./map/map-helpers.js";
import { createMapController } from "./map/map-controller.js?v=clima-stations-cleanup-20260717";
import { createMapRenderContext } from "./map/map-render-context.js?v=clima-stations-cleanup-20260717";
import {
    readBiofisicoState,
    syncBiofisicoStateFromLocals
} from "./state/biophysical-state.js";
...
import {
    zoomLandformsMapRenderer,
    zoomSoilOrderMapRenderer,
    zoomLandSuitabilityMapRenderer
} from "./map/biophysical-zoom-renderer.js";
import {
    clearBiofisicoHighlight,
    createDebouncedBiofisicoHighlight,
    highlightBiofisicoWhere
} from "./map/biophysical-highlight-renderer.js";
import { 
    createMainMap 
...
import {
   buildLegendFromRenderer,
    buildLandscapeDictionaryFromRenderer as buildPaisajeDictFromRendererRaw,
...
} from "./map/biophysical-legend-renderer.js?v=pendientes-imageserver-20260716";
import { applyWhereToLayers, createBiofisicoStationsLayer } from "./map/biophysical-map-renderer.js?v=clima-stations-cleanup-20260717";
import { createChartController } from "./charts/chart-controller.js?v=degradacion-domain-labels-v2-20260724";
import {
    destroyDualChartInstances,
    renderDualLandformCharts,
    renderDualLandSuitabilityCharts,
    toggleDualCharts
} from "./charts/chart-renderer.js?v=pie-guide-lines-direct-20260617";
import { getAxisTitles } from "./charts/chart-options.js";
import { DEGRADATION_ORDER, SEISMIC_ORDER } from "./charts/chart-utils.js";
import { createSoilOrderBubbleChart as createSoilOrderBubbleChartRaw } from "./charts/soils/soil-order-bubble-chart.js?v=orden-suelo-muni-height-20260617";
import { createTimeSlider } from "./ui/time-slider.js";
import {
    fetchLayerSource,
    fetchMunicipalityDictionary,
    fetchMunicipalityInfo
} from "./services/biophysical-query-service.js?v=territorial-catalog-20260727";
import {
    cachedQueryExtent,
    cachedQueryFeatures
} from "./services/biophysical-arcgis-cache-service.js";
import { clearBiofisicoRuntimeCaches } from "./services/biophysical-cache-service.js";
import {
    measureBiofisicoAsync,
    recordBiofisicoMetric
} from "./services/biophysical-performance-service.js";
import {
    soilOrderColors,
    ensureLandformDictionary,
    ensureSoilOrderDictionary,
    ensureLandSuitabilityDictionary,
    findLandSuitabilityCodeByLabel,
    findLandSuitabilityUseCodesByLabels,
...
import {
    ...
    STATIONS_LAYER_URL
} from "./config/biophysical-ui-config.js";
...
function clearLayers(options = {}) {
    ...
    clearClimateRiskMunicipalityHighlight();
...
function clearClimateRiskMunicipalityHighlight() {
    if (!climateRiskMunicipalityHighlightLayer) return;
    try {
        climateRiskMunicipalityHighlightLayer.removeAll();
        map?.remove(climateRiskMunicipalityHighlightLayer);
    } catch (_) {}
    climateRiskMunicipalityHighlightLayer = null;
}

async function highlightClimateRiskMunicipality(layer) {
    clearClimateRiskMunicipalityHighlight();

    const config = getActiveLayerConfig();
    if (!config?.isClima || config?.climaType !== "riesgo_cc" || !currentMunicipalityId) return;
    if (!layer || layer.destroyed || !map || !view || !GraphicsLayerCtor || !GraphicCtor) return;

    try {
        const q = layer.createQuery();
        const municipalityCode = String(currentMunicipalityId).trim();
        q.where = `mpcodigo = '${municipalityCode.replace(/'/g, "''")}'`;
        ...
        climateRiskMunicipalityHighlightLayer = new GraphicsLayerCtor({
            id: "riesgo-cc-municipio-highlight",
            listMode: "hide"
        });
        map.add(climateRiskMunicipalityHighlightLayer);
        climateRiskMunicipalityHighlightLayer.add(new GraphicCtor({
            geometry,
            symbol: {
                type: "simple-fill",
                color: [255, 255, 0, 0],
...
const municipalitiesByDepartmentCache = new Map();
...
let lastHoverWhere = "";
let climateRiskMunicipalityHighlightLayer = null;
...
function getDualChartDeps() {
    return {
        ...
        getGeoformasRendererDict: () => landformRendererDictionary
    };
}

function createDualLandSuitabilityCharts(options) {
    const refs = getDualChartRefs();
    renderDualLandSuitabilityCharts(options, refs, getDualChartDeps());
    commitDualChartRefs(refs);
}

function createDualLandformCharts(options) {
    const refs = getDualChartRefs();
    renderDualLandformCharts(options, refs, getDualChartDeps());
    commitDualChartRefs(refs);
}

async function zoomLandformsMap(paisajeValue, relieveValue = null) {
    ...
    if (landscapeReliefWhere) {
        if (await tryExtent(landscapeReliefWhere)) return;
    }

    if (await tryExtent(landscapeWhere)) return;
...
async function zoomLandSuitabilityMap(vocacionValue, usoValue = null) {
    const layer = layerGlobal;
    ...
    if (landSuitabilityUseWhere) {
        if (await tryExtent(landSuitabilityUseWhere)) return;
    }

    if (await tryExtent(landSuitabilityWhere)) return;
...
async function zoomSoilOrderMap(ordenValue, fertilityValue) {
    const layer = layerGlobal;
...
function setActiveVariantLayerByScale() {
    ...
    const legendTitle = (config.id === "geoformas")
        ? getLandformsScaleTitle(view.scale)
        : config.title;
    ...
    if (config.isHidro && config.hidroType === "cuencas" && desired.layerId === 20) {
        ...
        .then(r => r.json())
        .then(json => {
            window.cuencasDict = buildBasinsDictionaryFromRenderer(json);

            // si cambió la capa visible, actualizar gráfica
            if (changed) {
            const chartL = chartLayerGlobal || desired;
            chartL.when(() => window.updateChart?.(chartL, config));
            }
        });
...
async function loadMunicipalityDictionary() {
    try {
        const dictionary = await fetchMunicipalityDictionary();
        municipalityNames = dictionary.municipalityNames;
        departmentNames = dictionary.departmentNames;
        syncStateFromGlobals();
    } catch (e) {
...
require([
    ...
    const timeSliderApi = createTimeSlider({
        ...
        getDeforestacionPeriodoActivo: () => activeDeforestationPeriod,
        setDeforestacionPeriodoActivo: (v) => { activeDeforestationPeriod = v; },
        getDeforestacionPeriodosBase: () => deforestationBasePeriods,
        setDeforestacionPeriodosBase: (v) => { deforestationBasePeriods = v; },
        ...
        buildExtraWhere,
        buildDefinitionExpression,
        updateChart: (layer, config, opts) => updateChart(layer, config, opts),
    ...
    function renderSubTabs() {
        ...
        renderBiofisicoSubTabs({
            ...
            onSelectSubLayer(idx) {
                ...
                const selectedMunicipality = document.getElementById("municipios")?.value;
                const hasSelectedTerritory =
                    (!!selectedMunicipality && selectedMunicipality !== "0" && selectedMunicipality !== "COL") ||
                    (!!selectedDepartment && selectedDepartment !== "0" && selectedDepartment !== "COL") ||
                    !!currentMunicipalityId ||
                    (territoryLevel === "DEPTO" && !!currentDepartmentId);
    ...
    function clearSearch() {
        clearBiofisicoRuntimeCaches("clearSearch");
        hideTimeSlider();
        timeSliderTouched = false;
    ...
    function setMode(mode) {
        ...
        timeSliderTouched = false;
        const hasSelectedTerritory = currentMunicipalityId || (territoryLevel === "DEPTO" && currentDepartmentId);
        ...
        if (hasSelectedTerritory) {
            ...
            clearLegend("Leyenda");
            updateSummary();
            updateChartTitle(getActiveLayerConfig(), null, null);
        }
    }
    ...
    function updateSummary() {
        ...
        if (config?.id === "geoformas") {
            const selectedDept = String(document.getElementById("departamentos")?.value || currentDepartmentId || "").trim();
            ...
            if (selectedDept && selectedDept !== "0" && selectedDept !== "COL" && !selectedMuni) {
                const deptName = departmentNames?.[selectedDept] || departmentNames?.[currentDepartmentId] || selectedDept;
    ...
    function updateLegend(labels, colors, codes = null) {
        try {
            ...
            renderBiofisicoLegend({
                ...
                ordenDegradacion: DEGRADATION_ORDER,
                ordenSismica: SEISMIC_ORDER
            });
            bindLegendClickOnce();
    ...
    const stationsFactory = createBiofisicoStationsLayer({
        ...
        STATIONS_LAYER_URL,
        escapeHtml,
        getDiccionarioMunicipios: () => municipalityNames,
        getDiccionarioDepartamentos: () => departmentNames
    });
    ...
    function clearLandformInternalLegends() {
        const container = document.getElementById("geoformasChartLegend");
        if (container) container.remove();
    }

    function getLandformsLegendContainer() {
    ...
    function getLandformSegmentActiveCodes(segment, ring) {
        const st = window.__legendState;
        ...
        const landscapeCode = String(segment?.landscapeCode ?? segmentCode).trim();
        ...
        if (legendCode && allCodes.has(legendCode)) return [legendCode];
        if (allCodes.has(segmentCode)) return [segmentCode];
        if (landscapeCode && allCodes.has(landscapeCode)) return [landscapeCode];
        return segmentCode ? [segmentCode] : [];
    }

    function isLandformSegmentActive(segment, ring, activeCodes) {
        ...
        return (legendCode && activeCodes.has(legendCode)) ||
            activeCodes.has(segmentCode) ||
            activeCodes.has(landscapeCode);
    }

    function createLandformLegendItem({ segment, ring, color, active }) {
        ...
        item.addEventListener("click", async () => {
            ...
            const codes = getLandformSegmentActiveCodes(segment, ring);
    ...
    function renderLandformsChartLegend(chart) {
        if (!chart?.__geoformasMultiSeries) return;

        const container = getLandformsLegendContainer();
        if (!container) return;
        ...
        const buildPanel = ({ title, ring, dataset }) => {
            ...
            (dataset?.__segments || [])
                ...
                .forEach((segment, index) => {
                    list.appendChild(createLandformLegendItem({
                        segment,
                        ring,
                        color: dataset.__fullColors?.[index] || dataset.backgroundColor?.[index] || segment.color,
                        active: isLandformSegmentActive(segment, ring, activeCodes)
                    }));
                });
        ...
        const landscapeDataset = chart.data.datasets.find(dataset => dataset.__geoformasRing === "paisaje");
        const reliefDataset = chart.data.datasets.find(dataset => dataset.__geoformasRing === "relieve");
        const titles = chart.__multiSeriesLegendTitles || {
            inner: "Paisajes",
            outer: "Geoformas"
        };
        const visibleLandscapeCount = (landscapeDataset?.__segments || [])
            .filter(segment => Number(segment?.value) > 0 && String(segment?.label ?? "").trim())
            .length;
        const visibleReliefCount = (reliefDataset?.__segments || [])
            .filter(segment => Number(segment?.value) > 0 && String(segment?.label ?? "").trim())
            .length;
        const useFullWidthLegend = visibleLandscapeCount + visibleReliefCount > 14 || visibleReliefCount > 10;
        ...
        container.appendChild(buildPanel({
            title: titles.outer,
            ring: "relieve",
            dataset: reliefDataset
        }));
    }

    async function selectLandformChartSegment(dataset, segment) {
        if (!dataset || !segment) return false;

        const isLandscape = dataset.__geoformasRing === "paisaje";
        const ring = isLandscape ? "paisaje" : "relieve";
        const activeCodes = getLandformSegmentActiveCodes(segment, ring);
        ...
        if (activeConfig?.isSuelos && activeConfig?.suelosType === "vocacion") {
            if (isLandscape) {
                await zoomLandSuitabilityMap(segment.code);
            } else {
                await zoomLandSuitabilityMap(segment.landscapeCode, segment.code);
            }
        ...
        if (isLandscape) {
            await zoomLandformsMap(segment.code);
        } else {
            await zoomLandformsMap(segment.landscapeCode, segment.code);
        }
    ...
    function getClimateRiskMapBaseWhere(config) {
        if (!config?.isClima || config?.climaType !== "riesgo_cc" || territoryLevel !== "MUNI") return "";

        const departmentId = String(currentDepartmentId || "").trim();
        if (!departmentId) return "";

        return `dpcodigo = '${departmentId.replace(/'/g, "''")}'`;
    }
    ...
    async function applyLegendFilter() {
        ...
        const isBasinsLegend = config?.isHidro && config?.hidroType === "cuencas";
        const isEcosystemsLegend = config?.isEcosistema && config?.ecosistemaType === "ecosistemas";
        const isDeforestationLegend = config?.isEcosistema && config?.ecosistemaType === "deforestacion";
        const isSoilOrderLegend = config?.isSuelos && config?.suelosType === "orden";
        const isClimateRiskLegend = config?.isClima && config?.climaType === "riesgo_cc";
        const usesDefinitionLegendFilter = isBasinsLegend || isEcosystemsLegend || isDeforestationLegend || isSoilOrderLegend || isClimateRiskLegend;
        ...
        for (const currentLayer of targetLayers) {
            ...
            const baseWhereCurrent = String(getClimateRiskMapBaseWhere(config) || whereBase || "").trim();
    ...
    async function restartCurrentQuery() {
        const hasSelectedTerritory = Boolean(
            currentMunicipalityId || (territoryLevel === "DEPTO" && currentDepartmentId)
        );
    ...
    async function loadMunicipalities() {
        ...
        municipalities = Object.keys(municipalityNames)
            .map(codigo => {
                const normalizedCode = normalizeMunicipalityCode(codigo);
                return {
                    codigo: normalizedCode,
                    nombre: municipalityNames[codigo] || municipalityNames[normalizedCode] || normalizedCode,
                    depto: normalizeDepartmentCode(normalizedCode.substring(0, 2))
                };
            })
            .sort((a, b) => getMunicipalityDisplayName(a).localeCompare(getMunicipalityDisplayName(b), "es", { sensitivity: "base" }));
        municipalitiesByDepartmentCache.clear();
    ...
    function loadDepartments() {
        ...
        const uniqueDepartmentCodes = [...new Set(municipalities.map(m => m.depto))].sort((codigoA, codigoB) => {
    ...
    function renderMunicipalities(deptoFiltro = null) {
        ...
        const normalizedDepartment = deptoFiltro && deptoFiltro !== "0"
            ? normalizeDepartmentCode(deptoFiltro)
            : "";
        const cacheKey = normalizedDepartment || "__ALL__";

        let filteredMunicipalities = municipalitiesByDepartmentCache.get(cacheKey);
        if (!filteredMunicipalities) {
            filteredMunicipalities = normalizedDepartment
                ? municipalities.filter(m => m.depto === normalizedDepartment)
                : municipalities;
            municipalitiesByDepartmentCache.set(cacheKey, filteredMunicipalities);
        }
        ...
        filteredMunicipalities.forEach(muni => {
            ...
            opt.textContent = getMunicipalityDisplayName(muni);
    ...
    function createBiofisicoControllerApi() {
        function cancelScheduledCurrentLayerLoad() {
        ...
        return {
            ...
            loadCurrentLayer,
            cancelScheduledCurrentLayerLoad,
            shouldSkipDepartmentLayerRender() {
                return currentMode === "ECOSISTEMAS" && territoryLevel === "DEPTO" && !currentMunicipalityId;
            },
            getDepartmentLayerRenderDelay() {
                const config = getActiveLayerConfig();
                if (config?.isHidro && config?.hidroType === "cuencas" && territoryLevel === "DEPTO" && !currentMunicipalityId) {
                    return 700;
                }
                return 0;
            },
            ...
            setDepartmentFilter(deptoSeleccionado) {
                currentDepartmentId = normalizeDepartmentCode(deptoSeleccionado);
                territoryLevel = deptoSeleccionado && deptoSeleccionado !== "0" ? "DEPTO" : "";
            },
            setMunicipalityFilter(codigo) {
                const normalizedCode = normalizeMunicipalityCode(codigo);
                territoryLevel = "MUNI";
                currentMunicipalityId = normalizedCode;
                currentDepartmentId = normalizeDepartmentCode(normalizedCode.substring(0, 2));
            },
    ...
    bindTerritorySelectEvents({
        onDepartamentoChange: value => handleDepartmentSelectChange(createBiofisicoControllerApi(), value),
        onMunicipioChange: value => handleMunicipalitySelectChange(createBiofisicoControllerApi(), value)
    });


    function createSoilOrderBubbleChart(options) {
        destroyChartInstance();
        toggleLandformCharts(false);
        destroyLandformCharts();
        clearLandformInternalLegends();

        createSoilOrderBubbleChartRaw(options, {
    ...
    function resetChartVisualState() {
        ...
        chartCard?.classList.remove("chart-ecosistemas", "chart-hipsometria", "chart-hipsometria-depto", "chart-bubble", "chart-bubble-depto", "chart-bottom-legend");
        chartCard?.style.removeProperty("--biofisico-chart-height");
        chartCard?.style.removeProperty("--biofisico-hipso-depto-height");
        chartCard?.style.removeProperty("--biofisico-bottom-chart-height");
    ...
    function clearDepartmentHipsometryVisualState() {
        const canvas = document.getElementById("chart");
        const chartCard = canvas?.closest(".chart-card");
        chartCard?.classList.remove("chart-hipsometria", "chart-hipsometria-depto");
        chartCard?.style.removeProperty("--biofisico-hipso-depto-height");
        if (canvas) {
            canvas.style.removeProperty("max-height");
            canvas.style.removeProperty("min-height");
    ...
    function setBiofisicoLoadingMessage(message = "Cargando información...") {
        ...
        if (legendContent) {
            ...
            loadingMessage.className = "oot-js-biofisico-main-1";
            loadingMessage.textContent = message;
            legendContent.replaceChildren(loadingMessage);
            legendContent.classList.remove("collapsed");
            delete legendContent.dataset.legendSelectedCode;
            delete legendContent.dataset.legendFilterStatus;
            delete legendContent.dataset.legendFilterWhere;
    ...
    function createChart(labels, values, colors, type = 'bar', isVertical = false, datasets = null) {
        ...
        const isDepartmentOnlyPercentContext = () => {
            ...
            return Boolean(
                (territoryLevel === "DEPTO" || (deptValue && deptValue !== "0" && deptValue !== "COL")) &&
                !currentMunicipalityId &&
                !muniValue
            );
        };
        ...
        const applyDepartmentHipsometryCanvasHeight = () => {
            ...
            const finalHeight = Math.min(
                maximumHeight,
                Math.max(minimumHeight, chartChrome + (labelCount * rowHeight))
            );
            const chartCard = canvas.closest(".chart-card");
        ...
        if (type === 'bar' && !isVertical && !isEcosystemsChart) {
        ...
        const isLandformsMultiSeries = (layerConfig?.id === "geoformas" ||
        ...
        const isClimateLegendAtBottom =
            layerConfig?.isClima === true &&
            ["temp", "precip", "temp_cc", "precip_cc"].includes(layerConfig?.climaType);
        const isDegradationLegendAtBottom =
            layerConfig?.isFenomenos === true &&
            layerConfig?.fenomenosType === "degradacion";
        const hasBottomChartLegend =
            isClimateLegendAtBottom || isDegradationLegendAtBottom;
        ...
        const isStacked = !isLandformsMultiSeries && Array.isArray(datasets) && datasets.length > 0;
        ...
        const config = {
            ...
            plugins: {
                legend: {
                    ...
                    position: hasBottomChartLegend
                        ? 'bottom'
                        : (datasets ? 'right' : 'bottom'),
                    align: 'center',
                    labels: {
                        boxWidth: 12,
                        padding: hasBottomChartLegend ? 14 : 10,
                        color: hasBottomChartLegend ? "#526068" : undefined,
                        font: {
                            size: hasBottomChartLegend ? 11 : 10,
                            family: "Outfit",
                            weight: hasBottomChartLegend ? "400" : "normal",
                            lineHeight: 1.3
                        },
            ...
            onClick: async (evt, elements) => {
                ...
                if (datasets !== null && el.datasetIndex !== undefined) {
                    ...
                    if (layerConfig?.id === "geoformas" && segment) {
                        if (await selectLandformChartSegment(dataset, segment)) return;
                    }

                    const period = chartInstance.data.labels?.[el.index];
                    const periodText = Array.isArray(period) ? period.join(" ") : String(period ?? "");

                    if (dataset.rangeCode != null && periodText) {
                        filterByPeriodRange(dataset.rangeCode, periodText);
                        return;
                    }
                    if (dataset.rangeCode != null) {
                        filterByCodeRange(dataset.rangeCode);
                        return;
                    }
                }
        ...
        if (type === 'bar') {
            ...
            const chartPanelWidth = Math.max(
                0,
                Number(document.getElementById("chartDiv")?.clientWidth) ||
                Number(ctx?.canvas?.clientWidth) ||
                0
            );
            const shouldReduceBottomAxisLabels =
                hasBottomChartLegend &&
                totalLabels > 6 &&
                chartPanelWidth > 0 &&
                chartPanelWidth < 560;
            const bottomAxisTickLimit =
                chartPanelWidth < 380 ? 5 :
                chartPanelWidth < 480 ? 6 : 8;
            ...
            const hasDepartmentOnlySelection =
                ...
                !selectedMuniValue;
            const isDepartmentHypsometryChart =
                isHipsometryUiChart &&
                (territoryLevel === "DEPTO" || hasDepartmentOnlySelection);
            const hasManyDepartmentHypsometryRanges =
                isDepartmentHypsometryChart &&
                totalLabels >= 18;
            const hasFewHipsometryRanges =
                isHipsometryChart &&
                totalLabels <= 6;
            const usesHorizontalThreatAxisLabels =
                layerConfig?.isFenomenos === true &&
                ["inundaciones", "sismica"].includes(layerConfig?.fenomenosType);

            if (isVertical) {
                // BARRAS VERTICALES
                const bottomLegendLabelRotation = shouldReduceBottomAxisLabels
                    ? 25
                    : totalLabels <= 6
                    ? 0
                    : (totalLabels <= 9 ? 25 : 35);
                const verticalAxisLabelRotation = usesHorizontalThreatAxisLabels
                    ? 0
                    : hasBottomChartLegend
                        ? bottomLegendLabelRotation
                        : (isSmallScreen ? 65 : 45);
                const verticalAxisMinRotation = usesHorizontalThreatAxisLabels
                    ? 0
                    : hasBottomChartLegend
                        ? bottomLegendLabelRotation
                        : (isSmallScreen ? 65 : 35);
                ...
                config.options.scales = {
                    x: {
                        ...
                        title: {
                            ...
                            padding: { top: 14 },
                            font: hasBottomChartLegend
                                ? { size: 11, family: "Outfit", weight: "500" }
                                : undefined,
                            color: hasBottomChartLegend ? "#525b60" : undefined
                        },
                        ticks: {
                            autoSkip: usesHorizontalThreatAxisLabels
                                ? false
                                : shouldReduceBottomAxisLabels,
                            maxTicksLimit: usesHorizontalThreatAxisLabels
                                ? Math.max(totalLabels, 1)
                                : shouldReduceBottomAxisLabels
                                ? bottomAxisTickLimit
                                : (isVerySmallScreen ? 4 : isSmallScreen ? 5 : 8),
                            autoSkipPadding: shouldReduceBottomAxisLabels ? 12 : undefined,
                            maxRotation: verticalAxisLabelRotation,
                            minRotation: verticalAxisMinRotation,
                            padding: usesHorizontalThreatAxisLabels
                                ? 10
                                : (hasBottomChartLegend ? 8 : 6),
                            color: hasBottomChartLegend ? "#5c6469" : undefined,
                            font: {
                                size: usesHorizontalThreatAxisLabels
                                    ? 11
                                    : hasBottomChartLegend
                                    ? (isVerySmallScreen ? 9 : 11)
                    ...
                    y: {
                        ...
                        ticks: {
                            ...
                            color: hasBottomChartLegend ? "#5c6469" : undefined,
                            font: {
                                size: hasBottomChartLegend
                                    ? (isVerySmallScreen ? 9 : 10)
                                    : (isVerySmallScreen ? 9 : 10),
                                family: "Outfit",
                                weight: hasBottomChartLegend ? "400" : "normal",
                                lineHeight: 1.3
                ...
                if (isHipsometryChart) {
                    config.options.datasets = {
                        ...
                        bar: {
                            ...
                            maxBarThickness: hasFewHipsometryRanges
                                ? 28
                                : hasManyDepartmentHypsometryRanges
                                ? 9
                                : 16,
                            categoryPercentage: hasManyDepartmentHypsometryRanges ? 0.55 : 0.72,
                            barPercentage: hasFewHipsometryRanges
                                ? 0.86
                                : hasManyDepartmentHypsometryRanges
                                ? 0.68
                                : 0.82
                ...
                config.options.scales = {
                    ...
                    y: {
                        ...
                        ticks: {
                            ...
                            font: {
                                size: hasManyDepartmentHypsometryRanges
            ...
            if (chartCanvas) {
                ...
                if (!isVertical) {
                    ...
                    if (!isEcoChart && !isHipsometryChart) {
                        chartCard?.classList.remove("chart-hipsometria", "chart-hipsometria-depto");
                    }
                    ...
                    if (isEcoChart) {
                    ...
                    } else if (isDepartmentHypsometryChart) {
                        chartCard?.classList.remove("chart-hipsometria");
                        chartCard?.classList.add("chart-hipsometria-depto");
                        chartCard?.style.removeProperty("--biofisico-chart-height");
                        const minimumHeight = isSmallScreen ? 330 : 320;
                        const rowHeight = isSmallScreen ? 15 : 16;
                        ...
                        const dynamicHeight = Math.min(
                            maximumHeight,
                            Math.max(minimumHeight, chartChrome + (totalLabels * rowHeight))
                        );
                        chartCard?.style.setProperty("--biofisico-hipso-depto-height", `${dynamicHeight}px`);
                        chartCanvas.style.setProperty("height", `${dynamicHeight}px`, "important");
                        chartCanvas.style.setProperty("max-height", `${dynamicHeight}px`, "important");
                    } else {
                        chartCard?.classList.remove("chart-hipsometria", "chart-hipsometria-depto");
                        chartCard?.style.removeProperty("--biofisico-chart-height");
                        chartCard?.style.removeProperty("--biofisico-hipso-depto-height");
                    ...
                    chartCanvas.closest(".chart-card")?.classList.remove("chart-ecosistemas", "chart-hipsometria", "chart-hipsometria-depto");
                    ...
                    if (hasBottomChartLegend) {
                        const chartCard = chartCanvas.closest(".chart-card");
                        const containerWidth = Math.max(
                            320,
                            Number(chartCard?.clientWidth) || Number(chartCanvas.clientWidth) || 0
                        );
                        const legendItemsPerRow =
                            containerWidth >= 520 ? 4 :
                            containerWidth >= 380 ? 3 : 2;
                        const legendRows = Math.max(
                            1,
                            Math.ceil(chartDatasets.length / legendItemsPerRow)
                        );
                        const baseHeight = Math.max(
                            440,
                            Math.min(500, Math.round(containerWidth * 0.82))
                        );
                        const bottomLegendChartHeight = Math.min(
                            560,
                            baseHeight + ((legendRows - 1) * 26)
                        );
                        config.options.maintainAspectRatio = false;
                        chartCard?.classList.add("chart-bottom-legend");
                        chartCard?.style.removeProperty("min-height");
                        chartCard?.style.setProperty(
                            "--biofisico-bottom-chart-height",
                            `${bottomLegendChartHeight}px`
                        );
                        chartCanvas.style.removeProperty("height");
                        chartCanvas.style.removeProperty("max-height");
                    } else {
                        const chartCard = chartCanvas.closest(".chart-card");
                        chartCard?.classList.remove("chart-bottom-legend");
                        chartCard?.style.removeProperty("--biofisico-bottom-chart-height");
                        chartCard?.style.removeProperty("min-height");
                        chartCanvas.style.setProperty("height", isSmallScreen ? "360px" : "300px");
                        chartCanvas.style.removeProperty("max-height");
                    }
                }
            }
        ...
        if (isLandformsMultiSeries) {
            ...
            config.plugins = [
                ...(config.plugins || []),
                {
                    ...
                    afterUpdate(chart) {
                        renderLandformsChartLegend(chart);
                    }
                }
            ];
        }
        ...
        config.options.onClick = async (evt, elements) => {
            const activeElements = elements?.length
                ? elements
                : (isLandformsMultiSeries && chartInstance?.getElementsAtEventForMode
            ...
            if (activeElements?.length) {
                ...
                if (isLandformsMultiSeries) {
                    ...
                    if (segment && await selectLandformChartSegment(clickedDataset, segment)) return;
                }

                if (!isLandformsMultiSeries && await setOnlyChartCategoryActive(clickedLabel, idx, clickedDataset)) {
                    return;
                }
        ...
        if (!isEcosystemsChart) {
            ...
            if (!isDepartmentOnlyHipsometryContext()) {
                finalChartCard?.classList.remove("chart-hipsometria", "chart-hipsometria-depto");
        ...
        if (isLandformsMultiSeries) {
            ...
            window.__biofisicoApplyGeoformasChartFilter = (activeCodesInput) => {
                ...
                const reliefDataset = chartInstance.data.datasets.find(dataset => dataset.__geoformasRing === "relieve");
                ...
                chartInstance.data.datasets.forEach(dataset => {
                    ...
                    if (dataset.__geoformasRing === "paisaje") {
                        dataset.data = dataset.__segments.map((segment, index) => {
                            const relatedReliefs = (reliefDataset?.__segments || [])
                                .filter(relieve => String(relieve?.landscapeCode ?? "").trim() === String(segment?.code ?? "").trim());

                            if (relatedReliefs.length) {
                                return relatedReliefs.reduce((sum, relieve) => {
                                    return sum + (isLandformSegmentActive(relieve, "relieve", activeCodes)
                                        ? Number(relieve.value) || 0
                                        : 0);
                                }, 0);
                            }

                            return isLandformSegmentActive(segment, "paisaje", activeCodes)
                                ? Number(dataset.__fullData?.[index] ?? segment.value) || 0
                                : 0;
                        });
                    } else {
                        dataset.data = dataset.__segments.map((segment, index) => {
                            return isLandformSegmentActive(segment, "relieve", activeCodes)
                                ? Number(dataset.__fullData?.[index] ?? segment.value) || 0
                                : 0;
                        });
                    }
                ...
                renderLandformsChartLegend(chartInstance);
            };
    ...
    function buildLandscapeDictionaryFromRenderer(layer){
        const m = new Map();
    ...
    function buildLegendEntryFromAttrs(config, attrs, layer) {
        ...
        if (config.id === "pendientes") {
            ...
            const info = slopeColors?.[code] || slopeColors?.[Number(code)];
            return {
        ...
        if (config?.isSuelos && config.suelosType === "orden") {
            ...
            // 1) primero intenta tu diccionario de colores (biophysical-colors.js)
            let info = (soilOrderColors && soilOrderColors[code]) ? soilOrderColors[code] : null;
        ...
        if (config.isRadar) {
            return {
                fixed: true,
                labels: CLIMATE_RISK_LEGEND.map(i => i.label),
                colors: CLIMATE_RISK_LEGEND.map(i => i.color),
                codes: CLIMATE_RISK_LEGEND.map((_item, index) => String(index))
            };
        }

        if (config.isBF3) {
            const code = String(attrs.paisaje ?? "");
            const dict = buildLandscapeDictionaryFromRenderer(layer);
        ...
        if (config.isGeoforma) {
            ...
            const key = `${p},${t}`;
            const pair = landformRendererDictionary?.[`${p}||${t}`];
            const info = landformColors?.[key];
            const landscapeLabel = pair?.landscapeLabel || String(info?.label || "").split(",")[0]?.trim() || p;
            const reliefLabel = pair?.reliefLabel || String(info?.label || "").split(",")[1]?.trim() || t;
            const label = pair
                ? `${landscapeLabel} - ${reliefLabel}`
        ...
        if (config.isClima) {
            ...
            const info = climateColors?.[code];
            return { label: info?.label || code, color: info?.color || "#999", code };
            }

            let dict = {};
            if (config.climaType === "temp") dict = temperatureColors;
            if (config.climaType === "precip") dict = precipitationColors;
            if (config.climaType === "temp_cc") dict = temperatureChangeColors;
            if (config.climaType === "precip_cc") dict = precipitationChangeColors;
        ...
        if (config.isEcosistema) {
            ...
            const code = String(attrs[config.labelField] ?? "");
            const info = deforestationColors?.[code];
            return { label: info?.label || code, color: info?.color || "#999", code };
            }
        ...
        if (config.isSuelos) {
            ...
            const key = `${v},${u}`;
            const pair = landSuitabilityRendererDictionary?.[`${v}||${u}`];
            const info = landSuitabilityColors?.[key];
            const landSuitabilityLabel = pair?.landSuitabilityLabel || landSuitabilityMainDictionary?.[v]?.label || String(info?.label || "").split(",")[0]?.trim() || v;
            const usoLabel = pair?.usoLabel || String(info?.label || "").split(",")[1]?.trim() || u;

            return {
                label: `${landSuitabilityLabel} - ${usoLabel}`,
                color: pair?.color || info?.color || "#999",
                code: key
            };
            } else {
            const code = String(attrs[config.labelField] ?? "");
            const info = conflictColors?.[code];
            return { label: info?.label || code, color: info?.color || "#999", code };
            }
        }
        ...
        if (config.id === "hipsometria" || config.id === "hipsometria_depto" || config.isDeptoAgg) {
            const info = hypsometricColors?.[code];
            return { label: info?.label || code, color: info?.color || "#999", code };
        }
        ...
        try {
            ...
            if (config.isFenomenos && config.fenomenosType === "degradacion") {
                ...
                const ordered = entries.sort(
                    (a, b) => (DEGRADATION_ORDER[a.label] ?? 999) - (DEGRADATION_ORDER[b.label] ?? 999)
                );
            ...
            if (config.isRadar || config.isDeptoRiskCount) {
                updateLegend(
                    CLIMATE_RISK_LEGEND.map(i => i.label),
                    CLIMATE_RISK_LEGEND.map(i => i.color),
                    CLIMATE_RISK_LEGEND.map((_item, index) => String(index))
                );
                return;
            }

            if (config.id === "pendientes" && config.isPendientesPolar) {
                const entries = Object.entries(slopeColors)
    ...
    function getDepartmentBasinsGroupField(config, layer) {
    ...
    const chartControllerContext = {
        ...
        get territoryLevel() { return territoryLevel; },
        set territoryLevel(value) { territoryLevel = value; },
        get whereBase() { return whereBase; },
        set whereBase(value) { whereBase = value; },
        get currentDepartmentId() { return currentDepartmentId; },
        set currentDepartmentId(value) { currentDepartmentId = value; },
        get currentMunicipalityId() { return currentMunicipalityId; },
        set currentMunicipalityId(value) { currentMunicipalityId = value; },
        get departmentNames() { return departmentNames; },
        set departmentNames(value) { departmentNames = value || {}; },
        get municipalityNames() { return municipalityNames; },
        set municipalityNames(value) { municipalityNames = value || {}; },
    ...
    function buildBasinsDictionaryFromRenderer(layerJson) {
    ...
    const chartController = createChartController({
        ...
        getLandformColor,
        normKey,
        getColoresOrdenSuelo: () => soilOrderColors,
        getDeforestacionPeriodoActivo: () => activeDeforestationPeriod,
        getDeforestacionPeriodosBase: () => deforestationBasePeriods,
        setDeforestacionPeriodosBase: (value) => { deforestationBasePeriods = value; },
        measureBiofisicoAsync,
        recordBiofisicoMetric
    });

    const { updateChart } = chartController;
    ...
    mapRenderContext = createMapRenderContext({
        ...
        getWhereBase: () => whereBase,
        getCurrentMode: () => currentMode,
        getFiltroNivel: () => territoryLevel,
        getMunicipioActual: () => currentMunicipalityId,
        getDeptoActual: () => currentDepartmentId,
        getLayerGlobal: () => layerGlobal,
        getLayersGlobal: () => layersGlobal,
        ...
        updateChart,
        updateSource,
        highlightClimateRiskMunicipality,
        ensureStationsLayer,
        ensureLandformDictionary,
        ensureSoilOrderDictionary,
        cachedQueryExtent,
    ...
    const attrFilters = createAttributeFilters({
        ...
        sqlEquals,
        andWhere,
        landformColors: (typeof landformColors !== "undefined" ? landformColors : {}),
        ...
        landSuitabilityColors: (typeof landSuitabilityColors !== "undefined" ? landSuitabilityColors : window.landSuitabilityColors || {}),
    ...
    var { filterByPeriodRange, filterByCodeRange, filtrarPorAtributo } = attrFilters;
