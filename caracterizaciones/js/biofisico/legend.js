...
export function sortLegendEntries(config, entries) {
    ...
    if (config.isClima && config.climaType === "clima_tipo") {
        const order = Object.keys(climateColors || {}).sort((a,b)=>Number(a)-Number(b));
        const idx = new Map(order.map((c,i)=>[climateColors[c]?.label || c, i]));
    ...
    if (config.id === "hipsometria" || config.id === "hipsometria_depto" || config.isDeptoAgg) {
        const order = Object.keys(hypsometricColors || {}).map(Number).sort((a,b)=>a-b).map(String);
        const idx = new Map(order.map((c,i)=>[(hypsometricColors[c]?.label || c), i]));
        return entries.slice().sort((a,b)=>(idx.get(a.label) ?? 9999) - (idx.get(b.label) ?? 9999));
    }


    if (config.isHidro && config.hidroType === "escorrentia") {
        const inv = new Map(Object.entries(runoffColors || {}).map(([k,v])=>[v.label, Number(k)]));
