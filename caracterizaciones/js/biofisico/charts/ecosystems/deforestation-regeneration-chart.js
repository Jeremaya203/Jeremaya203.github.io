...
function renderDeforestationTimeSeries(ctx, features, deps = {}) {
    ...
    const defCode = "14001";
    const regCode = "14002";
    const fallbackDict = deps.deforestationColors || globalThis.deforestationColors || {};
    ...
    let basePeriods = deps.getDeforestacionPeriodosBase?.() || [];

    if (!basePeriods.length || activePeriod === "Todos") {
        basePeriods = [...orderedPeriodsCurrent];
        deps.setDeforestacionPeriodosBase?.(basePeriods);
    }

    const orderedPeriods = basePeriods.length
        ? [...basePeriods]
    ...
    for (const feature of features) {
        const attrs = feature.attributes || {};
        const period = String(attrs[periodField] ?? "").trim();
        const dynamicType = String(attrs[dynField] ?? "").trim();
        const value = Number(attrs[valueField]) || 0;

        if (!period || !byPeriod.has(period)) continue;

        const row = byPeriod.get(period);
        if (dynamicType === defCode) row.def += value;
        else if (dynamicType === regCode) row.reg += value;
    }
    ...
    ctx.createChart(orderedPeriods, [], null, "line", false, datasets);
...
export function deforestationMunicipalSeriesHandler(deps = {}) {
    return {
        when: (ctx) =>
        ...
        run: async (ctx) => {
            ...
            try {
                ...
                renderDeforestationTimeSeries(ctx, res.features, deps);
            } catch (e) {
...
export function forestDepartmentLineHandler(deps = {}) {
    return {
        when: (ctx) =>
        ...
        run: async (ctx) => {
            try {
                ...
                ctx.createChart(periods, [], [], "line", true, datasets);
