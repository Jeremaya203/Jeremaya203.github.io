...
export function runoffMunicipalHandler(deps = {}) {
    const runoffColors = deps.runoffColors || globalThis.runoffColors || {};
    ...
    return {
        ...
        run: async (ctx) => {
            ...
            try {
                ...
                const labels = codes.map(code => runoffColors?.[code]?.label || code);
                const values = codes.map(code => dataByCode[code] || 0);
                const colors = codes.map(code => runoffColors?.[code]?.color || "#999");

                ctx.createChart(labels, values, colors, "line", false);
            } catch (e) {
...
export function runoffDepartmentVerticalPercentageHandler(deps = {}) {
    return {
        when: (ctx) =>
