 ...
 */
export function createSoilOrderBubbleChart({ xLabels, yLabels, datasets, isDepartment = false }, deps) {
    const {
        getCanvas,
        getChartInstance,
        setChartInstance,
    ...
    async function syncPointWithMapAndLegend(chart, point) {
        ...
        const fertilityValue = point.xValue;


        await zoomSoilOrderMap(ordenValue, fertilityValue);
    }
    ...
    function calculateBubbleRadius(percentage) {
    ...
    function groupSoilOrderBubbles(datasets, xLabels, yLabels) {
        ...
        (datasets || []).forEach(ds => {
            ...
            (ds.data || []).forEach(p => {
                ...
                if (!xLabel || !yLabel || percentage <= 0) return;
                ...
                const item = groupedPoints.get(key);
                item.percentage += percentage;
            });
        });

        const groupedData = Array.from(groupedPoints.values()).map(item => ({
            ...item,
            r: calculateBubbleRadius(item.percentage)
        }));

        return [{
            label: "Distribución",
            data: groupedData,
            backgroundColor: groupedData.map(d => d.backgroundColor),
            borderColor: groupedData.map(d => d.borderColor),
            borderWidth: 1.5,
            hoverBorderWidth: 2
        }];
    }

    const totalPercentage = (datasets || []).reduce((acc, ds) => {
        return acc + (ds.data || []).reduce((sum, p) => sum + Number(p.percentage || 0), 0);
    }, 0);
    ...
    (datasets || []).forEach(ds => {
        (ds.data || []).forEach(p => {
            ...
            totalsByX[key] = (totalsByX[key] || 0) + Number(p.percentage || 0);
        });
    });

    const groupedDatasets = groupSoilOrderBubbles(datasets, xLabels, yLabels);
    const fullBubbleData = groupedDatasets[0]?.data ? [...groupedDatasets[0].data] : [];
    const fullBubbleBackground = Array.isArray(groupedDatasets[0]?.backgroundColor)
        ? [...groupedDatasets[0].backgroundColor]
        : groupedDatasets[0]?.backgroundColor;
    const fullBubbleBorder = Array.isArray(groupedDatasets[0]?.borderColor)
        ? [...groupedDatasets[0].borderColor]
        : groupedDatasets[0]?.borderColor;
    ...
    if (totalEl) {
        totalEl.textContent = `Total acumulado: ${totalPercentage.toFixed(1)}%`;
    }
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            ...
                            const value = Number(raw.percentage || 0);
                            const totalX = Number(totalsByX[xLabel] || 0);

                            return [
                                `${yLabel} / ${xLabel}: ${value.toFixed(2)}%`,
