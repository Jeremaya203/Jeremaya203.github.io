...
function renderSoilOrderBubble(ctx, features, deps) {
    const { yField, xField, valueField } = ctx.config.ordenAgg;
    const soilOrderColors = deps.getColoresOrdenSuelo?.() || deps.soilOrderColors || globalThis.soilOrderColors || {};
    const rendererDict = buildRendererDict(ctx.layer || ctx.lyr, deps);

    const fertilityInfo = (value) => {
    ...
    const ordenInfo = (value) => {
        ...
        const info = rendererInfo || soilOrderColors?.[key];
    ...
    for (const feature of features) {
        ...
        const fert = fertilityInfo(attrs[xField]);
    ...
    deps.createSoilOrderBubbleChart?.({
        xLabels,
        yLabels,
        datasets,
...
function renderSoilOrderSimple(ctx, features) {
    const data = {};
...
export function soilOrderMunicipalHandler(deps = {}) {
    return {
        when: (ctx) =>
            ctx.config?.isSuelos &&
        ...
        run: async (ctx) => {
            ...
            try {
                ...
                if (ctx.config?.isBubbleOrdenSuelo && ctx.config?.ordenAgg) {
                    renderSoilOrderBubble(ctx, features, deps);
                    return;
                }

                renderSoilOrderSimple(ctx, features);
            } catch (e) {
...
export function soilOrderDepartmentPieHandler(deps = {}) {
    return {
        when: (ctx) =>
