...
function getHypsometryColors(deps = {}) {
    const globalColors = typeof hypsometricColors !== "undefined"
        ? hypsometricColors
        : globalThis.hypsometricColors;
    const colors = deps.hypsometricColors || globalColors || {};
...
export function renderDualLandSuitabilityCharts({
    pieLabels,
    pieValues,
    pieColors,
    donutLabels,
    ...
    refs.geoPieChartInstance = new Chart(pieCanvas.getContext("2d"), withPiePercentageLabels({
        ...
        options: {
            ...
            onClick: async function (_evt, elements) {
                ...
                const landSuitabilityLabel = pieLabels[idx];

                window.__vocacionSelectedLabel = landSuitabilityLabel;

                const landSuitabilityCode = deps.findLandSuitabilityCodeByLabel(landSuitabilityLabel);
                if (!landSuitabilityCode) return;

                await deps.zoomLandSuitabilityMap(landSuitabilityCode);
                ...
                if (clickedLayer && !clickedLayer.destroyed) {
                    await deps.updateChart(clickedLayer, clickedConfig, { skipSyncMap: true });
                }
            },
    ...
    refs.geoDonutChartInstance = new Chart(donutCanvas.getContext("2d"), withPiePercentageLabels({
        ...
        options: {
            ...
            onClick: async function (_evt, elements) {
                ...
                const landSuitabilityLabel = selectedVocacion;

                const codes = deps.findLandSuitabilityUseCodesByLabels(landSuitabilityLabel, usoLabel);
                if (!codes) return;

                await deps.zoomLandSuitabilityMap(codes.vocacion, codes.usopvoc);
...
export function renderDualLandformCharts({
    landscapeSegments = [],
    reliefSegments = []
}, refs, deps) {
    toggleDualCharts(true);
    ...
    const innerColors = landscapeSegments.map(segment => segment.color);
    const outerColors = reliefSegments.map(segment => segment.color);

    refs.geoPieChartInstance = new Chart(pieCanvas.getContext("2d"), withPiePercentageLabels({
        type: "doughnut",
        data: {
            labels: [
                ...landscapeSegments.map(segment => segment.label),
                ...reliefSegments.map(segment => segment.label)
            ],
            datasets: [
                {
                    label: "Paisajes",
                ...
                },
                {
                    ...
                    data: reliefSegments.map(segment => segment.value),
        ...
        options: {
            ...
            onClick: async function (_evt, elements) {
                ...
                const isLandscape = element.datasetIndex === 0;
                const activeCodes = isLandscape ? (segment.childCodes || [segment.code]) : [segment.code];

                await deps.setCategoryCodesActive?.(activeCodes, "chart");

                if (isLandscape) {
                    await deps.zoomLandformsMap(segment.code);
                } else {
                    await deps.zoomLandformsMap(segment.landscapeCode, segment.code);
                }
    ...
    window.__biofisicoApplyGeoformasChartFilter = (activeCodesInput) => {
        ...
        chart.data.datasets.forEach((dataset, datasetIndex) => {
            dataset.backgroundColor = dataset.__segments.map((segment, index) => {
                const isLandscape = datasetIndex === 0;
                const visible = isLandscape
                    ? activeCodes.has(String(segment.code)) || (segment.childCodes || []).some(code => activeCodes.has(String(code)))
                    : activeCodes.has(String(segment.code)) || activeCodes.has(String(segment.landscapeCode));
