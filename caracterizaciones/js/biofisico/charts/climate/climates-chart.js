function getClimatesDict(deps = {}) {
    return deps.climateColors || globalThis.climateColors || {};
}
...
function getClimateInfo(code, rendererDict, climateColors) {
    const normalizedCode = normalizeCode(code);
    return rendererDict.get(normalizedCode) ||
        climateColors?.[normalizedCode] ||
        climateColors?.[String(code ?? "").trim()] ||
        null;
}
...
export function climatesMunicipalHandler(deps = {}) {
    const climateColors = getClimatesDict(deps);
        ...
        run: async (ctx) => {
            ...
            try {
                ...
                for (const feature of res.features) {
                    ...
                    const percentage = Number(attrs[ctx.config.valueField]) || 0;
                    dataByCode[code] = (dataByCode[code] || 0) + percentage;
                }

                const domainCodes = Array.from(new Set([
                    ...Object.keys(climateColors || {}).map(normalizeCode),
                ...
                const labels = domainCodes.map(code => getClimateInfo(code, rendererDict, climateColors)?.label || code);
                const values = domainCodes.map(code => dataByCode[code] || 0);
                const colors = domainCodes.map(code => getClimateInfo(code, rendererDict, climateColors)?.color || "#999");
...
export function climatesDepartmentPercentageHandler(deps = {}) {
    return {
        when: (ctx) =>
