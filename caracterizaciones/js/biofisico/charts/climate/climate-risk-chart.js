import { CLIMATE_RISK_LEGEND } from "../../config.js";
...
export function climateRiskMunicipalRadarHandler() {
    return {
        when: (ctx) =>
            ctx.config?.isClima &&
            ctx.config?.isRadar &&
        ...
        run: async (ctx) => {
            ...
            try {
                ...
                const values = [
                    ...
                    Number((sumRisk / count).toFixed(2))
                ];

                ctx.createChart(labels, values, "rgba(171, 65, 36, 0.5)", "radar", false);
            } catch (e) {
...
export function climateRiskDepartmentCountHandler() {
    return {
        when: (ctx) =>
        ...
        run: async (ctx) => {
            try {
                ...
                const buildWhere = () => {
                    ...
                    if (ctx.currentDepartmentId) {
                        const n = Number(ctx.currentDepartmentId);
                        if (Number.isFinite(n)) return `dpcodigo = ${n}`;
                        return `dpcodigo = '${String(ctx.currentDepartmentId).replace(/'/g, "''")}'`;
                    }
                    return "1=1";
                };
                ...
                const labels = CLIMATE_RISK_LEGEND.map(item => item.label);
                const colors = CLIMATE_RISK_LEGEND.map(item => item.color);
                const values = [0, 1, 2, 3, 4, 5].map(key => byRisk.get(key) || 0);

                ctx.createChart(labels, values, colors, "bar", true);
                ctx.updateChartTitle(ctx.config, null, null);
