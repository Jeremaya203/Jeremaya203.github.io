...
export class SubtabManager {
    ...
    async refresh(options = {}) {
        ...
        const completeAvailability = async () => {
            await this._refreshAvailability(abortController.signal);
            if (abortController.signal.aborted || cycleId !== this.refreshCycleId || mode !== this.state.get('currentMode')) return;
            this._ensureValidActiveIndex();
            this.render();
        };
        ...
        if (options.deferAvailability) {
            setTimeout(() => {
                if (abortController.signal.aborted || cycleId !== this.refreshCycleId) return;
    ...
    render() {
        ...
        const list = this.layerConfig.getList(mode, territoryLevel, condicionantesGroup);
        const rawList = this.layerConfig.getRawList(mode);
        ...
        const visibleTabs = this._getVisibleTabs(list);
    ...
    _getVisibleTabs(list = []) {
        return list
            .map((cfg, idx) => ({ cfg, idx }))
            .filter(({ cfg }) => !cfg.hideSubtab);
    }
    ...
    _renderGrouped(rawList, activeList, currentIdx) {
        ...
        rawList.filter(cfg => !cfg.hideSubtab).forEach((cfg) => {
    ...
    async _refreshAvailability(signal = null) {
        const mode = this.state.get('currentMode');
        const rawList = this.layerConfig.getRawList(mode);
        const needsAvailability = rawList.some(cfg => cfg.availability?.requiresData);
        ...
        const checks = await this._resolveAvailability(rawList, whereBase, signal);
    ...
    async _resolveAvailability(rawList, whereBase, signal = null) {
        ...
        await Promise.all([
            ...Array.from(groups.values()).map(group => this._applyGroupedAvailability(group, checks, signal)),
            ...fallback.map(cfg => this._applyFallbackAvailability(cfg, whereBase, checks, signal))
        ]);

        return checks;
    }

    async _applyGroupedAvailability(group, checks, signal = null) {
        try {
            const counts = await this.queryService.queryGroupedCounts(
                group.url,
                group.where,
                group.groupField,
                { signal }
    ...
    async _applyFallbackAvailability(cfg, whereBase, checks, signal = null) {
        ...
        try {
            ...
            const counts = await Promise.all(sources.map(source =>
                this.queryService.queryCount(source.url, where, { signal })
            ));
            if (signal?.aborted) return;
    ...
    _ensureValidActiveIndex() {
        const mode = this.state.get('currentMode');
        const territoryLevel = this.state.get('territoryLevel');
        const list = this.layerConfig.getList(mode, territoryLevel, this.state.get('condicionantesGroup'));
        const rawList = this.layerConfig.getRawList(mode);
    ...
    _resolveActiveGroupId(groups, activeCfg) {
        if (activeCfg?.hideSubtab) return null;
