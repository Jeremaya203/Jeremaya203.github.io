...
|   |-- layer-filter.js
|   |-- map-highlighter.js
|   |-- map-click-handler.js
|   |-- map-controls.js
|   |-- map-view-badge.js
|   `-- map-source-display.js
|
|-- overview/
|   |-- overview-map.js
|   `-- overview-drag-handler.js
|
|-- legend/
|   |-- legend-renderer.js
|   |-- legend-filter.js
|   `-- legend-data-extractor.js
|
|-- charts/
|   |-- chart-registry.js
|   |-- chart-factory.js
...
|   |-- chart-manager.js
|   `-- renderers/
|       |-- doughnut-renderer.js
|       |-- horizontal-bar-renderer.js
|       |-- stacked-bar-renderer.js
|       |-- pie-renderer.js
|       `-- LineaInfraestructuraRenderer.js
|
|-- ui/
|   |-- dropdown-manager.js
|   |-- downloads-dropdown.js
|   |-- subtab-manager.js
|   |-- territory-selector.js
|   |-- municipality-selector.js
|   |-- search-controls.js
...
|   `-- territory-data-service.js
|
|-- navigation/
|   |-- module-navigator.js
|   |-- mode-switcher.js
|   |-- sub-layer-navigator.js
...
- `event-bus.js`: canal de comunicacion entre dominios.

### `config/`

Contiene configuracion declarativa. Aqui se describe que existe, no como se renderiza en detalle.

- `layer-config.js`: tematicas, capas, campos, tipos de renderer, campos de categoria, campos de valor, filtros base y reglas de leyenda.
- `mode-config.js`: modos disponibles y su relacion con la UI.
- `domain-config.js`: dominios, etiquetas, colores y constantes compartidas.
...
```js
import { LayerFilter } from '../map/layer-filter.js';
...
1. Agregar la entrada en `layer-config.js`.
2. Declarar los bloques `map`, `chart`, `legend` y `filter` que apliquen.
3. Si usa dominios o colores compartidos, agregarlos en `domain-config.js`.
...
2. Registrar el tipo en `chart-registry.js` o en la configuracion equivalente.
...
1. Agregar el caso generico en `layer-factory.js`.
2. Declarar su uso desde `layer-config.js`.
...
1. Agregar el modo en `mode-config.js`.
2. Agregar sus tematicas en `layer-config.js`.
3. Agregar textos, dominios o colores en `domain-config.js` si aplica.
