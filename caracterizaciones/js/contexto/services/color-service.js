...
const DETERMINANTES = {
    ...
    ambientalesGeneral: {
        serviceId: 19,
        serviceName: "CL_DTS",
        query: "((tdeterm = 1 AND confactadm = 1) OR tdeterm IN (2,3,4,5,6,7,8))",
        renderer: {
            type: "unique-value",
            field: "tdeterm",
            alias: "Categoría ambiental"
        },
        ...
        colors: {
            "1": {
                ...
                hatch: {
                    ...
                    separation: 5,
                    lineWidth: 0.5,
                    assetUrl: new URL("../../../img/contexto/patterns/ambiental-sinap.svg", import.meta.url).href
                }
            },
            "2": {
                value: "2",
                ...
                hatch: {
                    ...
                    separation: 2,
                    lineWidth: 0.5,
                    assetUrl: new URL("../../../img/contexto/patterns/ambiental-ecosystem.svg", import.meta.url).href
                }
            },
            "3": {
                value: "3",
                label: "Estrategias Complementarias de Conservación",
                fillColor: "rgba(110, 169, 0, 0.63)",
                outlineColor: "#6EA900",
                outlineWidth: 0.5
            },
            "4": {
                value: "4",
                label: "Derivadas de Instrumentos de Planificación",
                fillColor: "#002673",
                outlineColor: "#002673",
                outlineWidth: 0.5,
                hatch: {
                    type: "circle-marker-dots",
                    separation: 0.75,
                    markerSize: 1.5,
                    lineWidth: 0.5,
                    assetUrl: new URL("../../../img/contexto/patterns/ambiental-planning-dots.svg", import.meta.url).href
                }
            },
            "5": {
                value: "5",
            ...
            "6": {
                value: "6",
                label: "Gestión del Recurso Suelo",
                fillColor: "#894465",
                outlineColor: "#653249",
                outlineWidth: 0.5
            },
            "7": {
                value: "7",
                label: "Gestión del Riesgo y Cambio Climático",
                fillColor: "#5A00B5",
                outlineColor: "#3E007D",
                outlineWidth: 0.5
            },
            "8": {
                value: "8",
                label: "Uso y Biodiversidad del Paisaje",
                fillColor: "#00EDD6",
                outlineColor: "#009E8F",
                outlineWidth: 0.5
            }
...
sitiosArqueologicos: {
    ...
    colors: {
        "1201": {
            ...
            outlineColor: "#cd8966",
            outlineWidth: 0.5,
            iconUrl: new URL("../../../img/patrimoniales/sitios_arqueologicos_house.svg", import.meta.url).href,
            symbol: {
                type: "icon",
                name: "house",
                width: 13,
...
puertosAeropuertosPunto: {
    ...
    colors: {
        "Puertos": {
            ...
            outlineColor: "#894e4e",
            outlineWidth: 0,
            iconUrl: new URL("../../../img/contexto/icons/port.svg", import.meta.url).href,
        ...
        "Aeropuertos": {
            ...
            outlineColor: "#894e4e",
            outlineWidth: 0,
            iconUrl: new URL("../../../img/contexto/icons/airport.svg", import.meta.url).href,
...
export class ColorService {
    ...
    getLegendItems(domainName) {
        return Object.values(this.getColors(domainName)).map(item => ({
            ...
            hatchAngle: item.hatch?.angle,
            hatchSeparation: item.hatch?.separation,
            hatchLineWidth: item.hatch?.lineWidth,
    ...
    _buildSymbol(symbolType, item) {
        ...
        if (item.hatch) {
            return this._buildPictureFillSymbol(item);
        }
    ...
    _buildPictureFillSymbol(item) {
        const hatch = item.hatch || {};
        const separation = Math.max(0.75, Number(hatch.separation || 5));
        const patternType = String(hatch.type || '').toLowerCase();
        const color = item.fillColor || item.outlineColor || '#000000';
        const isMarker = patternType.includes('shape') || patternType.includes('marker');
        const markerSize = Math.max(1, Number(hatch.markerSize || 1.5));
        const pictureSize = isMarker
            ? Math.max(5, markerSize + separation * 4)
            : separation;
        ...
        if (hatch.assetUrl) {
            return {
                ...
                height: pictureSize,
                outline: {
                    color: item.outlineColor || color,
                    width: Number(item.outlineWidth ?? 0.5)
                }
            };
        }

        // ArcGIS descarga los SVG embebidos de PictureFillSymbol mediante fetch.
        // Una URL data: queda bloqueada por connect-src; los patrones restantes
        // usan el relleno diagonal nativo y no necesitan conexiones adicionales.
        return {
            type: 'simple-fill',
            color,
    ...
    _getFillHatchStyle(hatch) {
        ...
        if (type.includes('shape') || type.includes('marker')) {
            return 'marker';
        }
    ...
    _registerDerivedDomains() {
        ...
        const lineFactorColors = Object.fromEntries(
