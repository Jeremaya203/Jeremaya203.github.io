/**
 * territory-data-service.js — Carga de Datos del Municipio
 *
...
export class TerritoryDataService {
    ...
    async loadInfo(code) {
        ...
        const url = `${this.BASE_URL}/query?where=mpcodigo='${code}'&outFields=*&returnGeometry=false&f=json`;
        const res = await fetch(url);
        const json = await res.json();

        const info = json.features?.[0]?.attributes || null;
        this._cache[code] = info;
        return info;
    }
}
