        ...
        this.items.forEach(item => {
            ...
            if (item.iconUrl) {
                ...
                if (item.symbolType === 'point') {
                    ...
                    const angle = Number(item.hatchAngle ?? 45);
                    const spacing = Math.max(3, Math.round(Number(item.hatchSeparation || 5) * 2));
                    const stroke = Math.max(1, Number(item.hatchLineWidth || 0.5) * 2);
                    swatch.style.background = '#fff';
                    if (item.hatchStyle === 'marker') {
                        swatch.classList.add('legend-color--marker-dots');
                        swatch.style.color = color;
                        for (let index = 0; index < 9; index++) {
                            const dot = document.createElement('i');
                            dot.className = 'legend-marker-dot';
                            dot.setAttribute('aria-hidden', 'true');
                            swatch.appendChild(dot);
                        }
                    } else {
                        swatch.style.backgroundImage = `repeating-linear-gradient(${angle + 90}deg, transparent 0 ${spacing}px, ${color} ${spacing}px ${spacing + stroke}px)`;
                    }
