...
export class LegendDataExtractor {
    ...
    extract(layer, config) {
        ...
        return infos.map((info) => {
            ...
            const colorInfo = this.colorService?.getColorInfo(colorDomain, value);
            const rendererLabel = String(info.label ?? '').trim();
            const rawValueLabel = String(value ?? '').trim();
            const label = rendererLabel && rendererLabel !== rawValueLabel
                ? rendererLabel
                : (colorInfo?.label || rendererLabel || rawValueLabel || 'Sin etiqueta');
            return {
