export class LayerConfig {
    ...
    _ambientalGeneralConfig() {
        return {
            id: 'determinantes_ambientales_general',
            mode: 'AMBIENTALES',
            title: 'General',
            hideSubtab: true,
            url: `${this.CL_BASE}/19`,
            ...
            map: {
                ...
                fieldType: 'integer',
                labelField: 'nomdet',
                colorDomain: 'ambientalesGeneral'
            },
            chart: {
                type: 'bar',
                renderer: 'HorizontalBarRenderer',
                dataStrategy: 'grouped-counts',
                fallbackCategoryCounts: true,
                deferSummary: true,
                title: 'Conteo general de determinantes ambientales',
                titleBase: 'Determinantes ambientales',
                includeTerritoryInTitle: true,
                showTitleOnlyWithData: true,
                loadingMessage: 'Cargando información...',
                noDataMessage: 'No hay determinantes ambientales para la consulta seleccionada.',
                categoryField: 'tdeterm',
                categoryLabelField: 'tdeterm',
                categoryOrder: [1, 2, 3, 4, 5, 6, 7, 8],
                colorDomain: 'ambientalesGeneral',
                selectionField: 'tdeterm',
                distinctCountField: 'iddts',
                valueLabel: 'Cantidad de determinantes',
                valueFormat: 'integer',
                categoryLabel: 'Categoría ambiental',
                wrapCategoryLabels: true,
                categoryLabelMaxLines: 4,
                forceCanvasBars: true,
                valueLabelPlacement: 'center-or-end',
                keepVisibleWhenSelectionEmpty: true,
                statistic: 'count'
            },
            ...
            summary: {
                ...
                distinct: true,
                groupField: 'tdeterm',
                groupColorDomain: 'ambientalesGeneral',
                itemLabelField: 'nomdet',
                relation: {
                    url: `${this.CL_BASE}/25`,
                    sourceKeyField: 'iddts',
                ...
                normativeField: 'tcldnorm',
                analysisField: 'tclanalisis',
                title: 'Detalle por categoría ambiental',
                emptyNormativeText: 'No registra descripción normativa.',
                emptyAnalysisText: 'No registra análisis temático.'
            },
            filter: {
                fixedWhere: '((tdeterm = 1 AND confactadm = 1) OR tdeterm IN (2,3,4,5,6,7,8))',
                territoryField: 'mpcodigo',
                departmentField: 'dpcodigo',
                categoryField: 'tdeterm',
                categoryFieldType: 'integer'
    ...
    getList(mode, territoryLevel, condicionantesGroup = null) {
        ...
        if (mode === 'CONDICIONANTES') {
            list = condicionantesGroup
                ...
                : [];
            if (condicionantesGroup === 'zomac' && !territoryLevel) return [];
        }
        ...
        if (territoryLevel === 'DEPTO') {
            const departmentOnly = list.filter(layer => this.DEPTO_ONLY_LAYER_IDS.has(layer.id));
            return departmentOnly.length ? departmentOnly : list.filter(layer => !this.DEPTO_ONLY_LAYER_IDS.has(layer.id));
        }
