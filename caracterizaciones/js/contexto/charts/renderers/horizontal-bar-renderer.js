export class HorizontalBarRenderer {
    ...
    _prepareCanvasLayout(canvas, items) {
        ...
        const isNationalView = !String(this.state?.get?.('territoryLevel') || '').trim();
    ...
    _draw(items, options = {}) {
        ...
        if (this._shouldUseHtmlBars(items)) {
        ...
        const colors = items.map(item => {
            ...
            const pattern = this._createCanvasPattern(canvas, item);
            if (!hasHighlight) return pattern || color;
    ...
    _shouldUseHtmlBars(items = []) {
        if (this.config?.chart?.forceCanvasBars) return false;
        return !!this.config?.chart?.useHtmlDenseLabels
            && (!!this.config?.chart?.alwaysUseHtmlBars || items.length > 6);
    }
    ...
    _drawHtmlBars(items, options = {}) {
        ...
        items.forEach(item => {
            ...
            if (item.hatchStyle) {
                const spacing = Math.max(4, Math.round(Number(item.hatchSeparation || 5) * 2));
                const stroke = Math.max(1, Number(item.hatchLineWidth || 0.5) * 2);
                fill.style.backgroundColor = '#fff';
                fill.style.backgroundImage = item.hatchStyle === 'marker'
                    ? `radial-gradient(circle, ${item.color || '#999'} 0 ${stroke}px, transparent ${stroke + 0.5}px)`
                    : `repeating-linear-gradient(135deg, transparent 0 ${spacing}px, ${item.color || '#999'} ${spacing}px ${spacing + stroke}px)`;
                if (item.hatchStyle === 'marker') {
                    fill.style.backgroundSize = `${spacing}px ${spacing}px`;
                }
            }
    ...
    _createCanvasPattern(canvas, item) {
        if (!item?.hatchStyle || !canvas?.ownerDocument) return null;

        const spacing = Math.max(4, Math.round(Number(item.hatchSeparation || 5) * 2));
        const size = Math.max(6, spacing + 2);
        const patternCanvas = canvas.ownerDocument.createElement('canvas');
        patternCanvas.width = size;
        patternCanvas.height = size;
        const context = patternCanvas.getContext('2d');
        const targetContext = canvas.getContext('2d');
        if (!context || !targetContext) return null;

        context.clearRect(0, 0, size, size);
        context.strokeStyle = item.color || '#999';
        context.lineWidth = Math.max(1, Number(item.hatchLineWidth || 0.5) * 2);
        context.beginPath();

        if (item.hatchStyle === 'marker') {
            context.arc(size / 2, size / 2, Math.max(1, context.lineWidth), 0, Math.PI * 2);
            context.fillStyle = item.color || '#999';
            context.fill();
        } else {
            context.moveTo(-size * 0.2, size * 0.8);
            context.lineTo(size * 0.8, -size * 0.2);
            context.moveTo(size * 0.2, size * 1.2);
            context.lineTo(size * 1.2, size * 0.2);
        }

        if (item.hatchStyle !== 'marker') context.stroke();
        return targetContext.createPattern(patternCanvas, 'repeat');
    }
