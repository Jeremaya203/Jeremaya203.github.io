...
export class ChartManager {
    constructor(state, eventBus, registry, factory, lifecycle, queryService, renderers = {}, colorService = null) {
        this.state = state;
    ...
    async render() {
        ...
        try {
            ...
            if (this._shouldUseGroupedStatistics(config)) {
                await this._renderGroupedStatisticsChart(
                    config,
                    where,
                    requestId,
                    titleOnlyWithData,
            ...
            if (chartCfg.dataStrategy === 'grouped-counts' && chartCfg.statistic === 'count') {
                await this._renderGroupedCountChart(
                    config,
                    where,
                    requestId,
                    titleOnlyWithData,
            ...
            if (prepared) {
                ...
                if (this.currentSummaryGroups.length) {
                    this._paintSummaryGroups(this._filterSummaryGroups(this.currentSummaryGroups, this.state.get('activeSelection')), config);
                } else {
                    this._scheduleDeferredSummary(config, where, requestId, abortController.signal, preparedKey);
                }
                return;
            }
            if (summaryCountData) {
            ...
            await this._renderSummary(features, config, requestId, abortController.signal).catch(error => {
                if (requestId !== this.requestId || abortController.signal.aborted || error?.name === 'AbortError') return;
    ...
    _buildItems(features, config) {
        ...
        (features || []).forEach(feature => {
            ...
            if (!groups.has(key)) {
                ...
                const colorInfo = this.colorService?.getColorInfo(domainName, colorKey ?? key);
                ...
                groups.set(key, {
                    ...
                    hatchAngle: legendItem?.hatchAngle ?? colorInfo?.hatch?.angle,
                    hatchSeparation: legendItem?.hatchSeparation ?? colorInfo?.hatch?.separation,
                    hatchLineWidth: legendItem?.hatchLineWidth ?? colorInfo?.hatch?.lineWidth,
    ...
    _shouldUseRawMunicipalPercentage(chartCfg) {
        return this.state.get('territoryLevel') === 'MUNI' && this._isPercentageValueChart(chartCfg);
    }

    _shouldAverageTerritoryPercentageChart(chartCfg) {
        const territoryLevel = this.state.get('territoryLevel');
        const isDepartmentOrNational = territoryLevel === 'DEPTO' || !territoryLevel;
    ...
    _getPercentageAverageDenominator() {
        const municipalities = this.state.get('municipalities') || [];
        if (!Array.isArray(municipalities)) return 0;

        if (this.state.get('territoryLevel') === 'DEPTO') {
            const currentDepartmentId = this.state.get('currentDepartmentId');
            if (!currentDepartmentId) return 0;
            return municipalities.filter((municipality) => String(municipality.depto) === String(currentDepartmentId)).length;
        }
    ...
    _getPreparedCacheKey(config, where) {
        ...
        return [
            config.id,
            this.state.get('territoryLevel') || 'NACIONAL',
            where || '1=1'
        ].join('::');
    }
    ...
    _shouldUseGroupedStatistics(config) {
        return config?.chart?.dataStrategy === 'grouped-stats' &&
            config.chart.statistic === 'value' &&
            this._shouldAverageTerritoryPercentageChart(config.chart);
    }

    async _renderGroupedStatisticsChart(config, where, requestId, titleOnlyWithData, signal) {
        const preparedKey = this._getPreparedCacheKey(config, where);
        const prepared = this._getPreparedCache(preparedKey);
        const items = prepared?.items || await this._queryGroupedStatisticItems(config, where, signal);
        if (requestId !== this.requestId) return;
        ...
        const summaryGroups = prepared?.summaryGroups || [];
        if (summaryGroups.length) {
            this.currentSummaryGroups = summaryGroups;
            this.currentSummaryConfig = config;
            this._paintSummaryGroups(
                this._filterSummaryGroups(summaryGroups, this.state.get('activeSelection')),
                config
            );
            return;
        }
        ...
        this._setPreparedCache(preparedKey, { items, summaryGroups: [] });
        this._scheduleDeferredSummary(config, where, requestId, signal, preparedKey);
    }

    async _queryGroupedStatisticItems(config, where, signal) {
        const chartCfg = config.chart || {};
        const categoryField = chartCfg.categoryField || config.filter?.categoryField;
        const colorField = chartCfg.colorValueField;
        const groupFields = [...new Set([categoryField, colorField].filter(Boolean))];
        const outputField = 'chart_total';
        const sources = Array.isArray(config?.sources) && config.sources.length
            ? config.sources
            : [{ url: config.url }];
        const results = await Promise.all(sources.map(source =>
            this.queryService.queryGroupedStatistics(
                source.url,
                where,
                [{
                    statisticType: chartCfg.serverStatistic || 'sum',
                    onStatisticField: chartCfg.valueField,
                    outStatisticFieldName: outputField
                }],
                groupFields,
                { signal }
            )
        ));
        const statisticFeatures = results.flat().map(feature => ({
            ...feature,
            attributes: {
                ...(feature.attributes || {}),
                [chartCfg.valueField]: feature.attributes?.[outputField]
            }
        }));
        return this._buildItems(statisticFeatures, config);
    }

    _scheduleDeferredSummary(config, where, requestId, signal, preparedKey) {
        if (!config?.summary?.enabled) return;
        this._cancelDeferredSummary();

        const delay = Math.max(500, Number(config.summary.deferMs || 2200));
        this.summaryTimer = setTimeout(async () => {
            this.summaryTimer = null;
            if (requestId !== this.requestId || signal?.aborted) return;
            try {
                const features = await this._querySummaryFeatures(config, where, signal);
                if (requestId !== this.requestId || signal?.aborted) return;
                await this._renderSummary(features, config, requestId, signal);
                if (requestId === this.requestId && this.currentSummaryGroups.length) {
                    this._setPreparedCache(preparedKey, {
                        items: this.currentItems,
                        summaryGroups: this.currentSummaryGroups
                    });
                }
            } catch (error) {
                if (requestId !== this.requestId || signal?.aborted || error?.name === 'AbortError') return;
                console.warn('[contexto2] ChartManager.deferredSummary:', error);
                this._setMessage('No fue posible consultar el detalle descriptivo.');
            }
        }, delay);
    }

    _cancelDeferredSummary() {
    ...
    async _renderGroupedCountChart(config, where, requestId, titleOnlyWithData, signal = null) {
        const preparedKey = this._getPreparedCacheKey(config, where);
        const prepared = this._getPreparedCache(preparedKey);
        const useDistinctServerCount = !!config.chart?.distinctCountField;
        const summaryCountData = prepared
            ? null
            : (!useDistinctServerCount && this._shouldUseSummaryCountMethod(config)
                ? await this._querySummaryCountItems(config, where, requestId, signal)
                : null);
        const items = prepared?.items || summaryCountData?.items || (useDistinctServerCount
            ? await this._queryGroupedDistinctCountItems(config, where, signal)
            : await this._queryGroupedCountItems(config, where, signal));
        if (requestId !== this.requestId) return;
        ...
        this.currentRenderer?.render(items, config);
        this._setMessage('');
        if (prepared?.summaryGroups?.length) {
            this.currentSummaryGroups = prepared.summaryGroups;
            this.currentSummaryConfig = config;
            this._paintSummaryGroups(
                this._filterSummaryGroups(prepared.summaryGroups, this.state.get('activeSelection')),
                config
            );
            return;
        }
        ...
        this._setPreparedCache(preparedKey, { items, summaryGroups: [] });
        if (config.chart?.deferSummary) {
            this._scheduleDeferredSummary(
                config,
                where,
                requestId,
                signal,
    ...
    async _queryGroupedDistinctCountItems(config, where, signal = null) {
        const chartCfg = config.chart || {};
        const categoryField = chartCfg.categoryField || config.filter?.categoryField;
        const distinctField = chartCfg.distinctCountField;
        const sources = Array.isArray(config?.sources) && config.sources.length
            ? config.sources
            : [{ url: config.url }];
        const requests = sources.map(source => this.queryService.queryGroupedStatistics(
            source.url,
            where,
            [{
                statisticType: 'count',
                onStatisticField: distinctField,
                outStatisticFieldName: 'chart_total'
            }],
            [categoryField, distinctField],
            { signal }
        ));
        const results = await Promise.all(requests);
        const distinctValuesByCategory = new Map();

        results.flat().forEach(feature => {
            const attrs = feature?.attributes || {};
            const category = attrs[categoryField];
            const distinctValue = attrs[distinctField];
            if (category == null || distinctValue == null || distinctValue === '') return;
            const categoryKey = String(category);
            if (!distinctValuesByCategory.has(categoryKey)) {
                distinctValuesByCategory.set(categoryKey, new Set());
            }
            distinctValuesByCategory.get(categoryKey).add(String(distinctValue).trim());
        });

        const domainName = chartCfg.colorDomain || config.map?.colorDomain;
        return this._sortChartItems([...distinctValuesByCategory.entries()]
            .map(([key, values], index) => {
                const colorInfo = this.colorService?.getColorInfo(domainName, key);
                return {
                    value: key,
                    colorValue: key,
                    ...
                    hatchAngle: colorInfo?.hatch?.angle,
                    hatchSeparation: colorInfo?.hatch?.separation,
                    hatchLineWidth: colorInfo?.hatch?.lineWidth,
                    outlineColor: colorInfo?.outlineColor,
                    hasRawMunicipalPercentage: false,
                    territoryTotals: new Map()
                };
            })
            .filter(item => item.total > 0), chartCfg);
    }

    async _queryGroupedCountItems(config, where, signal = null) {
        const chartCfg = config.chart || {};
        ...
        let grouped;
        try {
            grouped = await this.queryService.queryGroupedCountsMany(
                sources,
                where,
                groupField,
                { signal }
            ...
            if (signal?.aborted || !chartCfg.fallbackCategoryCounts) throw error;
            return this._queryCategoryCountItems(config, where, sources, groupField, signal);
        }
        ...
        return this._buildCountItemsFromTotals(totals, config);
    }

    async _queryCategoryCountItems(config, where, sources, groupField, signal = null) {
        const chartCfg = config.chart || {};
        const domainName = chartCfg.colorDomain || config.map?.colorDomain;
        const categories = Array.isArray(chartCfg.categoryOrder)
            ? chartCfg.categoryOrder
            : Object.keys(this.colorService?.getColors(domainName) || {});
        const fieldType = config.map?.fieldType || config.filter?.categoryFieldType || 'integer';
        const numericTypes = new Set(['integer', 'small-integer', 'long', 'single', 'double', 'number']);
        const quoteValue = value => numericTypes.has(fieldType) && !Number.isNaN(Number(value))
            ? String(Number(value))
            : `'${String(value).replace(/'/g, "''")}'`;

        const totals = await Promise.all(categories.map(async category => {
            const categoryWhere = SqlUtils.combine(where, `${groupField} = ${quoteValue(category)}`);
            const counts = await Promise.all(sources.map(source =>
                this.queryService.queryCount(source.url, categoryWhere, { signal })
            ));
            return [String(category), counts.reduce((sum, count) => sum + Number(count || 0), 0)];
        }));

        return this._buildCountItemsFromTotals(
            new Map(totals.filter(([, total]) => Number(total) > 0)),
            config
        );
    }

    _buildCountItemsFromTotals(totals, config) {
        const chartCfg = config.chart || {};
        const domainName = chartCfg.colorDomain || config.map?.colorDomain;
    ...
    _shouldUseSummaryCountMethod(config) {
        return config?.chart?.statistic === 'count' &&
            !config.chart.deferSummary &&
            !config.chart.countFromFeatures &&
            config?.summary?.enabled &&
            !!config.summary.groupField &&
            !!config.summary.itemLabelField;
    }

    async _querySummaryCountItems(config, where, requestId, signal = null) {
        const features = await this._querySummaryFeatures(config, where, signal);
        if (requestId !== this.requestId) return null;
        return this._buildSummaryCountDataFromFeatures(features, config, requestId, signal);
    }
    ...
    async _querySummaryFeatures(config, where, signal = null) {
        const summaryCfg = config.summary || {};
        ...
        const options = {
            ...(summaryCfg.distinct ? { returnDistinctValues: 'true' } : {}),
            ...(config.mode === 'AMBIENTALES'
                ? { parallelPaging: true, maxParallelPages: 3 }
                : {}),
            ...(signal ? { signal } : {})
        };
        const results = await Promise.all(sources.map(source =>
    ...
    _getSymbolType(domainName, colorInfo = null) {
        const domain = this.colorService?.getDomain(domainName);
    ...
    _getHatchStyle(colorInfo) {
        ...
        if (type.includes('shape') || type.includes('marker') || type.includes('dot')) {
            return 'marker';
        }
    ...
    _getTerritoryTitleSuffix(features = []) {
        ...
        if (territoryLevel === 'MUNI') {
            const municipalityName = this._normalizeTerritoryLabel(
                this._formatSummaryText(attrs.mpnombre) || this._getSelectedOptionText('municipios'),
                attrs.mpcodigo || this.state.get('currentMunicipalityId'),
                'MUNI'
            );
            const departmentName = this._normalizeTerritoryLabel(
                this._formatSummaryText(attrs.dpnombre) || this._getSelectedOptionText('departamentos'),
                attrs.dpcodigo || this.state.get('currentDepartmentId'),
                'DEPTO'
            );
            if (this._isSameTerritoryLabel(municipalityName, departmentName)) return municipalityName;
            return [municipalityName, departmentName].filter(Boolean).join(', ');
        }
    ...
    _isNationalNamesOnlySummary() {
        return !String(this.state.get('territoryLevel') || '').trim();
    }
    ...
    _buildSummaryGroups(features, config, relatedByKey = new Map()) {
        ...
        (features || []).forEach(feature => {
            ...
            if (!groups.has(key)) {
                const colorInfo = this.colorService?.getColorInfo(summaryCfg.groupColorDomain, key);
