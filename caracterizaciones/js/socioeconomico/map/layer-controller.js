...
import { createBoundedCache, destroyCanvasChart } from "../charts/core/chart-lifecycle.js";
import { setChartStatus } from "../charts/ui/chart-status.js";
import {
...
export function createLayerController({ state, deps }) {
    ...
    function isDepartmentPopupOnlyConfig(config) {
        return config?.chartConfig?.allowDepartmentPopup === true
            && state.territoryLevel === "DEPTO"
            && !String(state.currentMunicipalityId || "").trim();
    }

    function getConnectivityDepartmentCode() {
        const departmentCode = String(state.currentDepartmentId || "").trim();
        if (departmentCode) return departmentCode;

        const municipalityCode = String(state.currentMunicipalityId || "").trim();
    ...
    function getConnectivityContextWhere(config) {
        ...
        if (!isConnectivityConfig(config) || state.territoryLevel !== "MUNI" || !municipalityWhere || municipalityWhere === "1=1") {
            return mapWhere;
        }
    ...
    async function ensureConnectivityDepartmentBoundary(config) {
        const shouldShowDepartmentBoundary = isConnectivityConfig(config)
            || (isDepartmentContextConfig(config) && state.territoryLevel !== "MUNI");
    ...
    async function ensureConnectivityMunicipalityBoundary(config) {
        if (!isConnectivityConfig(config) || state.territoryLevel !== "MUNI") {
    ...
    async function queryConnectivityNavigationExtent(config, fallbackLayer) {
        if (((isConnectivityConfig(config) || isDepartmentContextConfig(config)) && state.territoryLevel !== "MUNI" || useDepartmentMapContextForMunicipality(config)) && connectivityDepartmentBoundaryLayer) {
    ...
    function normalizeChartWhere(config) {
        ...
        if (state.territoryLevel === "MUNI") return;
        if (!departmentField || !state.currentDepartmentId || !state.territoryLevel) return;
    ...
    async function refreshLayerLegend(layer, config) {
    ...
    async function renderChart(layer, config, options) {
        if (typeof updateChart === "function") {
            await updateChart(layer, config, options);
            return;
        }

        console.warn("updateChart no definida");
    }
    ...
    async function loadCurrentLayer() {
        ...
        if (config.skipMapLayer) {
            ...
            if (config.chartConfig && typeof updateChart === "function") {
        ...
        if (config.type === "table-layer" && config.mapFallback?.url) {
            ...
            (async () => {
                ...
                try {
                    await refreshLayerLegend(mapLayer, config);
                } catch (e) {
        ...
        if (Array.isArray(config.variants) && config.variants.length) {
            ...
            (async () => {
                ...
                const variantEntries = availableVariants.flatMap(variant => {
                    ...
                    const usesConnectivityMunicipalitySplit = isConnectivityConfig(config) && state.territoryLevel === "MUNI";
                ...
                updateMapViewBadge(config.title);
                updateSource(state.layerGlobal);
                deps.updateSummary();
                ...
                if (config.key === "PIB_DEPARTMENT" && typeof deps.renderActivePibSubitem === "function") {
                    ...
                    try {
                        await refreshLayerLegend(state.layerGlobal, config);
                    } catch (e) {
                        if (isExpectedLayerError(e)) return;
        ...
        (async () => {
            ...
            if (!mapPriority) deps.updateSummary();
            ...
            newLayer.when(async () => {
                ...
                if (config.forceRendererLegend) {
                    try {
                        await refreshLayerLegend(newLayer, config);
                    } catch (e) {
                        if (isExpectedLayerError(e)) return;
    ...
    async function loadCurrentLandUsePlanning() {
        ...
        if (state.currentMunicipalityId) {
            landUsePlanningWhere = `${filterField} = '${String(state.currentMunicipalityId).replace(/'/g, "''")}'`;
        } else if (state.territoryLevel === "DEPTO" && state.currentDepartmentId) {
            if (config.deptoFilterField) {
                landUsePlanningWhere = `${config.deptoFilterField} = '${String(state.currentDepartmentId).replace(/'/g, "''")}'`;
            } else if (
                filterField.toLowerCase() === "mpcodigo" ||
                filterField.toLowerCase() === "mp_codigo"
            ) {
                landUsePlanningWhere = `SUBSTRING(${filterField},1,2) = '${String(state.currentDepartmentId).replace(/'/g, "''")}'`;
            }
        }
        const currentCycle = ++state.renderCycleId;
        const newLayer = new FeatureLayer({
            url: config.url,
            definitionExpression: landUsePlanningWhere,
            outFields: config.outFields || ["*"],
            opacity: 0.85,
            visible: true
        });
        ...
        newLayer.when(async () => {
            ...
            if (config.ordenamientoType === "zonificacion_rural") {
                await renderZonificacionRuralCharts(newLayer, config, landUsePlanningWhere);
                if (currentCycle !== state.renderCycleId || state.layerGlobal !== newLayer || newLayer.destroyed) return;

            } else if (config.ordenamientoType === "areas_actividad") {
                await renderAreasActividadCharts(newLayer, config, landUsePlanningWhere);
                if (currentCycle !== state.renderCycleId || state.layerGlobal !== newLayer || newLayer.destroyed) return;

            } else if (config.ordenamientoType === "clasificacion_suelo") {
                await renderClasificacionSueloCharts(newLayer, config, landUsePlanningWhere);
                if (currentCycle !== state.renderCycleId || state.layerGlobal !== newLayer || newLayer.destroyed) return;

            } else {
                await updateChart(newLayer, config, { skipSyncMap: true });
                if (currentCycle !== state.renderCycleId || state.layerGlobal !== newLayer || newLayer.destroyed) return;
            }
