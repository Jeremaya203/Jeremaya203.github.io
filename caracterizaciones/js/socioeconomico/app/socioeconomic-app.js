import {
    LAYERS_CONFIG,
} from "../config.js?v=education-health-axis-labels-20260724";
import { INFRAESTRUCTURA_COMPLEMENTARIA_BAR_CHART_CONFIG } from "../charts/configs/infrastructure-charts-config.js?v=education-health-axis-labels-20260724";
import { PUBLIC_SERVICES_RADAR_CHART_CONFIG } from "../charts/configs/conditions-charts-config.js?v=telecom-diccionario-url-20260707";
...
import { buildFilteredLegendFromLayers, buildLegendFromRenderer } from "../ui/legend/legend-renderer-service.js?v=health-legend-dedupe-20260602";
import { getRendererLegendItems, getRendererVisualForValue } from "../charts/core/chart-symbol-utils.js?v=fluvial-colors-20260515";
import { createMunicipalDataController } from "../services/municipal-data.js?v=territorial-catalog-20260727";
...
import { createLegendFilterController } from "../ui/legend/legend-filters.js?v=livestock-map-legend-no-chart-sync-20260604";
import { createLegendRenderer } from "../ui/legend/legend-renderer.js?v=support-infrastructure-legend-patterns-20260603";
import { getSocioeconomicoRanges } from "../ui/color.js";
import { createUiControlsController } from "../ui/ui-controls.js?v=conditions-mode-fix-20260519";
import { createSliderController } from "../ui/slider-controls.js?v=app-orchestrator-phase1-20260507";
import { createMapCoreController } from "../map/map-core.js?v=ferrea-legend-toggle-20260511";
import { createMapEnvironment } from "../map/map-environment.js";
...
import { updateSource } from "../map/map-helpers.js?v=error-cleanup-20260509";
...
import { createLayerController } from "../map/layer-controller.js?v=health-points-layer-20260721";
import { createPibBarChartController } from "../charts/gdp-bar-chart.js?v=equipment-selection-two-20260724";
import { createPibSectorPieChartController } from "../charts/gdp-sector-pie-chart.js?v=company-sector-thin-selection-20260724";
import { createPibEmpresasStackedBarChartController } from "../charts/gdp-businesses-stacked-bar-chart.js?v=company-sector-compact-spacing-20260724";
import { createCensoPecuarioDoughnutController } from "../charts/dynamics/livestock-census-doughnut-chart.js?v=adaptive-internal-pie-labels-20260724";
import { createEvaPieChartController } from "../charts/dynamics/eva-pie-chart.js?v=adaptive-internal-pie-labels-20260724";
import { prepareVisibleChartCanvas, setChartTitle } from "../charts/ui/chart-panel.js?v=local-chart-title-20260529";
import {
    chartRequiresMunicipality,
    hasMunicipalitySelection,
    showMunicipalityRequiredChartState
} from "../charts/ui/municipality-required-state.js?v=global-municipality-required-state-20260604";
import { initializeWindowState } from "../state/socioeconomico-state.js?v=infra-complementaria-20260511";
import { setState } from "../state/store.js";
import { applyWhereToLayers } from "../map/layer-runtime.js?v=ferrea-legend-toggle-20260511";
...
let departmentNames = {};
let municipalities = []; // Array de {código, nombre, depto}
let layersGlobal = [];
...
function navigateToExternalComponent(targetPage, tab) {
    ...
    if (territory.departmentId) params.set("depto", territory.departmentId);
    if (territory.municipalityId) params.set("id", territory.municipalityId);
...
const {
    ...
    // updateChart: (...args) => updateChart(...args)
    updateChart: (...args) => updateChart(...args)
});
...
const municipalDataController = createMunicipalDataController({
    FeatureLayer,
    layersConfig: LAYERS_CONFIG,
    getMunicipalityNames: () => municipalityNames,
    setMunicipalityNames: (value) => { municipalityNames = value; },
    getDepartmentNames: () => departmentNames,
    setDepartmentNames: (value) => { departmentNames = value; },
    getMunicipalities: () => municipalities,
    setMunicipalities: (value) => { municipalities = value; }
});

const summaryController = createSummaryController({
    getTerritoryLevel: () => territoryLevel,
    getCurrentMunicipalityId: () => currentMunicipalityId,
    getCurrentDepartmentId: () => currentDepartmentId,
    getMunicipalityInfo: () => municipalityInfo,
    setMunicipalityInfo: (value) => { municipalityInfo = value; },
    getActiveLayerConfig,
    hideTimeSlider,
...
const { loadMunicipalities, loadDepartments, renderMunicipalities } = municipalDataController;
const { loadMunicipalityInfo, updateSummary } = summaryController;
const { updateLegend } = legendRenderer;
...
const {
    ...
    destroyChart,
    handleActivityAreasMapClick: manejarClickMapaAreasActividadBase,
...
function selectMunicipalityFromMapClick(municipalityCode) {
    ...
    if (![...selectMuni.options].some(option => option.value === municipalityCode)) {
        renderMunicipalities(departmentCode);
    }
...
async function handlePibDepartmentMunicipalityMapClick(event) {
    if (!isPibActive() || !["DEPTO", "MUNI"].includes(String(territoryLevel || ""))) return false;
    if (!view) return false;

    const activeDepartmentCode = String(currentDepartmentId || currentMunicipalityId?.slice?.(0, 2) || "").trim();
    ...
    if (municipalityCode === String(currentMunicipalityId || "").trim()) return false;

    return selectMunicipalityFromMapClick(municipalityCode);
}

async function handlePovertyDepartmentMunicipalityMapClick(event) {
    if (!isPovertyLevelActive() || !["DEPTO", "MUNI"].includes(String(territoryLevel || ""))) return false;
    if (!view) return false;
...
async function handleSupportInfrastructureMunicipalityMapClick(event) {
    if (!isSupportInfrastructureActive() || !["DEPTO", "MUNI"].includes(String(territoryLevel || ""))) return false;
    if (!view) return false;
...
async function handleDepartmentContextMunicipalityMapClick(event) {
    ...
    if (config?.keepDepartmentMapOnMunicipality !== true
        || !["DEPTO", "MUNI"].includes(String(territoryLevel || ""))
        || !view) {
        return false;
    }
...
function getPibFlowMode() {
    return territoryLevel === "MUNI" && String(currentMunicipalityId || "").trim()
        ? "valor-agregado"
        : "pib";
}
...
function resolvePibDepartmentCode() {
    ...
    const departmentCode = String(currentDepartmentId || "").trim();
...
function resolvePibVisibleMapWhere() {
    const isMunicipalValueAdded = window.__pibMapVariantKey === "VALOR_AGREGADO"
        && territoryLevel === "MUNI"
        && String(currentMunicipalityId || "").trim();
...
function resetTerritorialSelects() {
    ...
    if (departmentSelect) {
        ...
        if (departmentSelect.value !== "0") departmentSelect.value = "";
    }
...
async function syncPovertyInitialTerritoryFromSelects(config = getActiveLayerConfig()) {
    ...
    if (selectedMunicipality) {
        ...
        await loadMunicipalityInfo(selectedMunicipality);
        return true;
    }
...
const layerState = {
    ...
    get currentMunicipalityId() { return currentMunicipalityId; }, set currentMunicipalityId(value) { currentMunicipalityId = value; },
    ...
    get territoryLevel() { return territoryLevel; }, set territoryLevel(value) { territoryLevel = value; },
    get currentDepartmentId() { return currentDepartmentId; }, set currentDepartmentId(value) { currentDepartmentId = value; },
    get legendFilterLabel() { return legendFilterLabel; }, set legendFilterLabel(value) { legendFilterLabel = value; }
};


const { loadCurrentLayer } = createLayerController({
    state: layerState,
    deps: {
        FeatureLayer,
        getActiveLayerConfig,
...
window.__resetSocioSearch = clearSearch;
...
async function restartCurrentQuery() {
    ...
    if (config.key === "POVERTY_LEVEL") {
        ...
        const departmentName = departmentSelect?.options?.[departmentSelect?.selectedIndex]?.textContent?.trim() || "";

        if (selectedMunicipality) {
            currentMunicipalityId = selectedMunicipality;
            currentDepartmentId = selectedMunicipality.slice(0, 2) || selectedDepartment;
            territoryLevel = "MUNI";
            whereBase = `mpcodigo = '${selectedMunicipality.replace(/'/g, "''")}'`;
...
function syncSocioTerritoryStateFromSelects(departmentSelect, selectMuni, municipalityId = "", departmentId = "") {
    const nav = globalThis.ModuleNavigation;
    const resolvedDepartment = nav?.resolveDeptoSelectValue?.(departmentSelect, departmentId)
        || nav?.normalizeDeptoId?.(departmentId)
        || departmentId
        || (municipalityId ? municipalityId.substring(0, 2) : "");

    if (resolvedDepartment && Array.from(departmentSelect?.options || []).some(option => option.value === resolvedDepartment)) {
        departmentSelect.value = resolvedDepartment;
        currentDepartmentId = resolvedDepartment;
        renderMunicipalities(resolvedDepartment);
    }

    if (municipalityId && selectMuni) {
        territoryLevel = "MUNI";
        currentMunicipalityId = municipalityId;
        if (!currentDepartmentId && municipalityId.length >= 2) {
            currentDepartmentId = municipalityId.substring(0, 2);
        }
...
async function applySocioeconomicoUrlContext() {
    ...
    let departmentId = urlContext.departmentId || "";
    const municipalityId = urlContext.municipalityId || "";

    if (!departmentId && municipalityId) {
        departmentId = nav.normalizeDeptoId?.(municipalityId.substring(0, 2)) || municipalityId.substring(0, 2);
    }

    const departmentSelect = document.getElementById("departamentos");
    const selectMuni = document.getElementById("municipios");
    if (!departmentSelect || !selectMuni) return;

    const hasTerritory = Boolean(municipalityId || departmentId);
    ...
    if (municipalityId) {
        syncSocioTerritoryStateFromSelects(departmentSelect, selectMuni, municipalityId, departmentId);
    ...
    if (departmentId) {
        syncSocioTerritoryStateFromSelects(departmentSelect, selectMuni, "", departmentId);
        const resolvedDepartment = nav.resolveDeptoSelectValue?.(departmentSelect, departmentId) || departmentId;
        if (!Array.from(departmentSelect.options).some(option => option.value === resolvedDepartment)) {
            console.warn("No se pudo autoseleccionar el departamento desde la URL:", departmentId);
            return;
        }
        departmentSelect.value = resolvedDepartment;
...
async function bootstrapSocieconomicoApp() {
    ...
    const hasTerritoryUrl = Boolean(urlContext.municipalityId || urlContext.departmentId);
...
const { bindSelectEvents } = createSelectsController({
    ...
    getChartInstance: () => chartInstance,
    setMunicipioActual: (value) => { currentMunicipalityId = value; },
    setMunicipalityInfo: (value) => { municipalityInfo = value; },
    setFiltroNivel: (value) => { territoryLevel = value; },
    setDeptoActual: (value) => { currentDepartmentId = value; },
    getView: () => view
});
...
window.__refreshSocioSummary = updateSummary;
