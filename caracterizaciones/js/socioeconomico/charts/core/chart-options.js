...
export function createBarChartOptions({
    ...
    function resolveTickLabel(value, index) {
        ...
        const labelForAxis = chartConfig?.xAxis?.labelStrategy === "abbreviated"
            ...
            : sourceLabel;
        const formattedLabel = formatChartLabel(labelForAxis, {
        ...
        const staggerThreshold = Number.isFinite(chartConfig?.xAxis?.staggerLabelsThreshold)
            ? chartConfig.xAxis.staggerLabelsThreshold
            : Number.POSITIVE_INFINITY;
        const shouldStagger = chartConfig?.xAxis?.staggerLabels === true
            && rawLabels.length >= staggerThreshold
            && labelIndex % 2 === 1;

        if (!shouldStagger) return formattedLabel;

        const lines = Array.isArray(formattedLabel) ? formattedLabel : [formattedLabel];
        const staggerLines = Number.isFinite(chartConfig?.xAxis?.staggerLineCount)
            ? Math.max(1, Math.round(chartConfig.xAxis.staggerLineCount))
            : 2;
        return [...Array(staggerLines).fill(" "), ...lines];
    }
    ...
    return {
        ...
        scales: {
            ...
            y: {
                ...
                title: { display: Boolean(yTitle), text: yTitle },
                grid: { display: false },
                afterFit: Number.isFinite(yAxisWidth)
                    ? axis => {
                ...
                ticks: {
                    ...
                    font: {
                        size: yTickFontSize,
                        weight: yTickFontWeight,
                        lineHeight: yTickLineHeight
                    }
                }
            }
        },
...
export function createPieChartOptions({
    ...
    let lastHoverLabel = "";
    const isNarrowViewport = Number(document.documentElement?.clientWidth || window.innerWidth || 0) <= 768;
    return {
        responsive: true,
        maintainAspectRatio: false,
        layout: {
            padding: isNarrowViewport
                ? { top: 6, right: 22, bottom: 4, left: 22 }
