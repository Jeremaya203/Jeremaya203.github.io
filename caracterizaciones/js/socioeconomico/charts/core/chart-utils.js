...
export function getChartBaseWhere({
    ...
    if (chartConfig?.filter?.requiredLevel && territoryLevel !== chartConfig.filter.requiredLevel) return "1=0";
    ...
    if ((territoryLevel === "DEPTO" || territoryLevel === "MUNI") && currentDepartmentId) {
        const departmentName = chartConfig?.filter?.valueSource === "code"
            ? currentDepartmentId
            : (departmentNames?.[currentDepartmentId] || currentDepartmentId);
...
export function getDepartmentDisplayLabel(value, departmentNames = {}) {
    const raw = String(value ?? "").trim();
    return departmentNames?.[raw] || raw;
}

export function getCurrentDepartmentChartLabel(rows = [], currentDepartmentId, departmentNames = {}) {
    if (!currentDepartmentId) return "";
    const departmentName = departmentNames?.[currentDepartmentId] || currentDepartmentId;
    return findChartRowLabel(departmentName, rows) || findChartRowLabel(currentDepartmentId, rows);
}
