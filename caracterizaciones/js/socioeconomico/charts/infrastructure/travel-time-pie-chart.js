import { getChartBaseWhere, getChartFields } from "../core/chart-utils.js?v=travel-time-pie-20260511";
import { getChartSymbolLookups, getRendererLegendItems, getRendererVisualForValue } from "../core/chart-symbol-utils.js?v=travel-time-pie-20260511";
...
import { MUNICIPALITY_REQUIRED_CHART_MESSAGE, hasMunicipalitySelection } from "../ui/municipality-required-state.js?v=global-municipality-required-state-20260604";
...
export function createTravelTimePieChartController({
    ...
    async function updateChart(layer, config, options = {}) {
        ...
        if (!options.skipSyncMap && typeof window.updateLegend === "function") {
            window.updateLegend(labels, colors, rows.map(row => row.rawLabel), {
