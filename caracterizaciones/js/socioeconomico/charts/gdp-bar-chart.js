import { createChartCore } from "./core/chart-core.js";
import { createChartInteractions } from "./chart-interactions.js?v=travel-municipality-scope-20260721";
import { createBarChartRenderer } from "./renderers/bar-chart-renderer.js?v=connectivity-tab-resize-20260716";
import { createTravelTimePieChartController } from "./infrastructure/travel-time-pie-chart.js?v=adaptive-internal-pie-labels-20260724";
import { createEducationBarChartController } from "./infrastructure/education-bar-chart.js?v=education-staggered-axis-20260724";
import { createEquipamientosChartController } from "./infrastructure/facilities-chart.js?v=equipment-selection-two-20260724";
import { getRendererLegendItems, getRendererVisualForValue } from "./core/chart-symbol-utils.js?v=connectivity-line-style-legend-20260602";
import { createPublicServicesRadarChartController } from "./conditions/public-services-radar-chart.js?v=department-map-municipality-selection-20260717";
import { createPovertyGroupedBarChartController } from "./poverty-grouped-bar-chart.js?v=poverty-first-entry-visible-message-20260604";
import { createEconomicActivitiesBarChartController } from "./dynamics/economic-activities-bar-chart.js?v=economic-no-scroll-20260707";
import {
    chartRequiresMunicipality,
    hasMunicipalitySelection,
    showMunicipalityRequiredChartState
} from "./ui/municipality-required-state.js?v=global-municipality-required-state-20260604";

// Dynamic charts configs
import { createLivestockCensusBarChartController } from "../charts/dynamics/livestock-census-bar-chart.js?v=global-safe-zoom-labels-20260604";
import { createGreenBusinessFloatingBarChartController } from "../charts/dynamics/green-business-floating-bar-chart.js?v=irrigation-pointer-pan-20260604";
import { createEvaPieChartController } from "./dynamics/eva-pie-chart.js?v=adaptive-internal-pie-labels-20260724";
...
export function createPibBarChartController({
    ...
    getCurrentMunicipalityId,        // ← NUEVO: agregar este parámetro
    getDepartmentNames,
    getLayerGlobal,
    getLayersGlobal,
    getChartLayerGlobal,
    ...
    function isConnectivityDepartmentMode(config) {
        return hasConnectivityTabs(config) && getTerritoryLevel?.() !== "MUNI" && Boolean(getCurrentDepartmentId?.());
    }
    ...
    function isMunicipalityRequiredWithoutSelection(config) {
        return config?.key !== "CONNECTIVITY"
            ...
            && !hasMunicipalitySelection({ getTerritoryLevel, getCurrentMunicipalityId });
    }
    ...
    function buildConnectivityContextWhereForChart(chartConfig) {
        ...
        const baseWhere = String(getWhereBase?.() || "1=1").trim() || "1=1";
        if (getTerritoryLevel?.() !== "MUNI" || !baseWhere || baseWhere === "1=1") {
            return mapWhere;
        }
    ...
    function renderActiveChartSoon(delay = 250) {
        chartCore.schedule(async () => {
            ...
            if (isEquipamientosChart(config.chartConfig)) {
                ...
                await equipamientosController.updateChart(equipamientosLayer, config);
                return;
            }
            if (isAttributeCategoryChart(config.chartConfig)) {
                hideConnectivityTabs();
                ...
                await educationBarController.updateChart(educationLayer, config, renderOptions);
                return;
            }
            if (povertyBarController.isPovertyGroupedChart?.(config.chartConfig)) {
                hideConnectivityTabs();
                ...
                await povertyBarController.updateChart(povertyLayer, config);
                return;
            }
            if (config.chartConfig.type === "pie") {
                hideConnectivityTabs();
                ...
                if (!pieLayer) return;
                await travelTimePieController.updateChart(pieLayer, config, renderOptions);
                return;
            }
            if (config.chartConfig.type === "radar") {
                hideConnectivityTabs();
                ...
                await publicServicesRadarController.updateChart(radarLayer, config, renderOptions);
                return;
            }
            if (isGreenBusinessChart(config.chartConfig)) {
                hideConnectivityTabs();
                setActiveChartContext(config.chartConfig);
                chartInteractions.installDefaultLegendHooks?.();
                await greenBusinessController.updateChart(resolveChartLayer(config, config.chartConfig), config);
                return;
            }
    ...
    async function updateChart(layer, config, options = {}) {
     ...
     if (config?.chartConfig?.type === "livestockCensusBar") {
        setActiveChartContext(config.chartConfig);
        chartInteractions.installDefaultLegendHooks?.();
        await livestockCensusBarController.updateChart(layer, config);
        return;
    }
        if (config?.chartConfig?.type === "greenBusinessFloatingBar") {
        setActiveChartContext(config.chartConfig);
        chartInteractions.installDefaultLegendHooks?.();
        await greenBusinessController.updateChart(layer, config);
        return;
    }
        if (config?.chartConfig?.type === "economicActivitiesBar") {
            setActiveChartContext(config.chartConfig);
            await economicActivitiesBarController.updateChart(
        ...
        if (isEquipamientosChart(config?.chartConfig)) {
            setActiveChartContext(config.chartConfig);
            await equipamientosController.updateChart(layer, config, options);
            return;
        }
        ...
        if (isAttributeCategoryChart(config?.chartConfig)) {
            setActiveChartContext(config.chartConfig);
            chartInteractions.installDefaultLegendHooks?.();
            await educationBarController.updateChart(layer, config, options);
            return;
        }
        ...
        if (povertyBarController.isPovertyGroupedChart?.(config?.chartConfig)) {
            setActiveChartContext(config.chartConfig);
            chartInteractions.installDefaultLegendHooks?.();
            await povertyBarController.updateChart(layer, config, options);
            return;
        }
        ...
        if (config?.chartConfig?.type === "pie") {
            setActiveChartContext(config.chartConfig);
            chartInteractions.installDefaultLegendHooks?.();
            await travelTimePieController.updateChart(layer, config, options);
            return;
        }

        if (config?.chartConfig?.type === "radar") {
            setActiveChartContext(config.chartConfig);
            await publicServicesRadarController.updateChart(layer, config, options);
            return;
        }

        chartInteractions.installDefaultLegendHooks?.();
        await barRenderer.updateChart(layer, config);
    }
