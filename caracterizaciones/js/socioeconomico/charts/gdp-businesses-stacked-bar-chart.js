import { EMPRESAS_SECTOR_STACKED_BAR_CONFIG } from "./configs/businesses-charts-config.js?v=economic-sector-shared-colors-20260603";
import { renderHorizontalStackedBarChart } from "./renderers/bar-chart-renderer.js?v=company-sector-economic-axis-style-20260724";

const EMPRESAS_MIN_HEIGHT = 260;
const EMPRESAS_MAX_HEIGHT = 680;
...
function empresasLabelFontSize(rowCount, wrappedLabels = []) {
...
function empresasBarThickness(rowCount) {
...
function empresasBarSpacing(rowCount) {
...
function empresasYAxisWidth(wrappedLabels = [], fontSize = 9) {
...
export function createPibEmpresasStackedBarChartController({
    ...
    function resetEmpresasChartViewport(canvas, rows = []) {
        ...
        const rowHeight = rowCount <= 4 ? 38 : rowCount <= 10 ? 28 : 26;
        const height = Math.min(
            EMPRESAS_MAX_HEIGHT,
            Math.max(EMPRESAS_MIN_HEIGHT, rowCount * rowHeight + 115)
        );
    ...
    function departmentLabel(fieldInfo, attrs) {
        const rawValue = attrs.dpnombre || attrs.dpcodigo || getCurrentDepartmentId?.();
    ...
    function municipalityLabel(fieldInfo, attrs) {
        const rawValue = attrs.mpnombre || attrs.mpcodigo || getCurrentMunicipalityId?.();
        const metadataLabel = domainLabel(fieldInfo, rawValue);
        const code = String(rawValue ?? "").trim();
        if (/^\d{5}$/.test(code)) return selectOptionLabel("municipios", code) || metadataLabel;
        return metadataLabel || selectOptionLabel("municipios", getCurrentMunicipalityId?.());
    }
    ...
    async function buildReadableTitle(attrs = {}) {
        ...
        if (getTerritoryLevel?.() === "DEPTO" && !getCurrentMunicipalityId?.()) {
    ...
    function buildWhere() {
        ...
        if (municipalityId) {
            return `${EMPRESAS_SECTOR_STACKED_BAR_CONFIG.filterFields.municipality} = '${String(municipalityId).replace(/'/g, "''")}'`;
        }
        const departmentId = getCurrentDepartmentId?.();
        if (departmentId && getTerritoryLevel?.() === "DEPTO") {
            return `${EMPRESAS_SECTOR_STACKED_BAR_CONFIG.filterFields.department} = '${String(departmentId).replace(/'/g, "''")}'`;
        }
        return "";
    }
    ...
    async function queryAnalysisText() {
        ...
        if (!municipalityId || !textSource?.url || !textSource?.field) return "";
        const params = new URLSearchParams({
            f: "json",
            where: `${textSource.filterField} = '${String(municipalityId).replace(/'/g, "''")}'`,
    ...
    async function renderForCurrentFilter() {
        ...
        try {
            ...
            const maxLineLength = rows.length > 16 ? 22 : rows.length > 10 ? 24 : rows.length > 7 ? 26 : 30;
            const wrappedLabels = labels.map(label => wrapLabelLines(
                label,
                maxLineLength,
                Number.POSITIVE_INFINITY
            ));
            const barThickness = empresasBarThickness(rows.length);
            const barSpacing = empresasBarSpacing(rows.length);
            const yTickFontSize = empresasLabelFontSize(rows.length, wrappedLabels);
            ...
            chartInstance = renderHorizontalStackedBarChart({
                ...
                yTickLineHeight: 1.2,
                yTickPadding: 5,
                yAxisWidth: empresasYAxisWidth(wrappedLabels, yTickFontSize),
