import { setChartTitle, prepareVisibleChartCanvas, configureScrollableChartViewport } from "../ui/chart-panel.js?v=local-chart-title-20260529";
import { setChartStatus } from "../ui/chart-status.js";
import { createZoomOptions, ensureZoomKeepsVisibleData } from "../core/chart-options.js?v=global-safe-zoom-labels-20260604";
...
export function createGreenBusinessFloatingBarChartController({
    ...
    async function updateChart(layer, config) {
        ...
        const municipalityCode = String(getCurrentMunicipalityId?.() || selectedMunicipalityCode()).trim();
        const hasMunicipalityFilter = /\bmpcodigo\s*=/i.test(String(getWhereBase?.() || ""));
        const hasMunicipality = /^\d{5}$/.test(municipalityCode)
            && (getTerritoryLevel?.() === "MUNI" || hasMunicipalityFilter);
        if (!hasMunicipality) {
            chartCore?.destroyChart?.();
