import { CENSO_PECUARIO_MULTI_DOUGHNUT_CONFIG } from "../configs/dynamics-charts-config.js?v=global-municipality-required-state-20260604";
import { renderMultiSeriesDoughnutChart } from "../renderers/doughnut-chart-renderer.js?v=adaptive-internal-pie-labels-20260724";
...
export function createCensoPecuarioDoughnutController({
    ...
    async function queryDepartmentFeatures(departmentCode) {
        const departmentField = CENSO_PECUARIO_MULTI_DOUGHNUT_CONFIG.filterFields.department;
        return queryFeatures(`${departmentField} = '${String(departmentCode).replace(/'/g, "''")}'`);
    }

    async function queryMunicipalityFeature(municipalityCode) {
        const municipalityField = CENSO_PECUARIO_MULTI_DOUGHNUT_CONFIG.filterFields.municipality;
        const rows = await queryFeatures(`${municipalityField} = '${String(municipalityCode).replace(/'/g, "''")}'`);
        return rows[0] || null;
    }
    ...
    async function renderForCurrentFilter() {
        ...
        try {
            const municipalityFeature = await queryMunicipalityFeature(municipalityCode);
            ...
            const attrs = municipalityFeature.attributes || {};
            const departmentCode = attrs.dpcodigo || String(municipalityCode).slice(0, 2) || getCurrentDepartmentId?.();
            const departmentFeatures = await queryDepartmentFeatures(departmentCode);
            const municipalTotals = sumAnimalValues([municipalityFeature]);
