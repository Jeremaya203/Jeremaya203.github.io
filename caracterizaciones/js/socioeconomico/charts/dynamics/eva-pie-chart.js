...
import { buildAdaptiveFont, createZoomOptions, ensureZoomKeepsVisibleData, formatChartLabel } from "../core/chart-options.js?v=global-safe-zoom-labels-20260604";
import { configureScrollableChartViewport, prepareVisibleChartCanvas, setChartTitle } from "../ui/chart-panel.js?v=local-chart-title-20260529";
...
export function createEvaPieChartController({
    ...
    async function queryFeatures(municipalityCode) {
        ...
        const params = new URLSearchParams({
            f: "json",
            where: `${currentConfig.filterField || "mpcodigo"} = '${String(municipalityCode).replace(/'/g, "''")}'`,
    ...
    async function renderForCurrentFilter() {
        ...
        try {
            const features = await queryFeatures(municipalityCode);
            if (!features.length) {
                destroyCharts();
                if (pieTitle) pieTitle.textContent = currentConfig.title || "Distribución de ciclo productivo";
                if (areaTitle) areaTitle.textContent = "Área sembrada por tipo de cultivo";
