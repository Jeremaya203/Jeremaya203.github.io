...
// Patrón idéntico a biophysical-colors.js
// =============================================================================
...
const urbanCenterDensityColors = ["#A80000", "#AF1831", "#B63162", "#BC4993", "#C362C4", "#CA7AF5"];
const ruralDensityColors = ["#EDFCD4", "#D4EECE", "#9FDAB8", "#D0BC94", "#C79430", "#A86600"];

const nationalDensityBreaks = {
    urbano: [
...
function buildDensityBreaksFromUpperValues(values = []) {
    return values.map((max, index) => {
        ...
        return {
            max,
            label: index === 0
                ? `<= ${formatDensityNumber(max, decimals)} (hab/ha)`
                : `${formatDensityNumber(previous, decimals)} - ${formatDensityNumber(max, decimals)} (hab/ha)`
        };
    });
}

function resolveDensityPaletteColor(palette, index, breakCount, { alignTail = false } = {}) {
...
function getDensityBreaksForDepartment(departmentCode) {
    const normalized = String(departmentCode || "").padStart(2, "0");
    const spec = departmentDensityBreaks[normalized];
    if (!spec) return nationalDensityBreaks;

    const breaks = {
        urbano: buildDensityBreaksFromUpperValues(spec.urbano || []),
        rural: buildDensityBreaksFromUpperValues(spec.rural || [])
    };
...
function getDensityArcadeBreakData(breakItems = []) {
    const limits = breakItems.map(item => Number(item.max));
    ...
    return { limits, mins, inclusive };
}

function resolveDensityBreakIndex(breakItems = [], value) {
...
function buildDensityZoneClassExpression(limitesName, minsName, inclusiveName, prefixExpr) {
    return `
            for (var i = 0; i < Count(${limitesName}); i++) {
                if (IIF(${inclusiveName}[i] == 1, density >= ${minsName}[i], density > ${minsName}[i]) && density <= ${limitesName}[i]) {
...
function buildPopulationDensityRenderer(departmentCode = null) {
    const breaks = departmentCode ? getDensityBreaksForDepartment(departmentCode) : nationalDensityBreaks;
    const urbanArcade = getDensityArcadeBreakData(breaks.urbano || []);
    const ruralArcade = getDensityArcadeBreakData(breaks.rural || []);
    const infos = [];
    ...
    (breaks.urbano || []).forEach((item, index) => {
        const color = resolveDensityPaletteColor(urbanCenterDensityColors, index, urbanoCount, { alignTail: true });
        infos.push({
            value: `C${index + 1}`,
    ...
    (breaks.rural || []).forEach((item, index) => {
        infos.push({
            ...
            symbol: {
                type: "simple-fill",
                color: resolveDensityPaletteColor(ruralDensityColors, index, ruralCount),
                outline: { color: "#ffffff", width: 0.35 }
            }
        });
    });
    ...
    return {
        ...
        valueExpression: `
            ...
            var density = Number($feature.denpobha);
            var urbanLimits = ${JSON.stringify(urbanArcade.limits)};
            var urbanMins = ${JSON.stringify(urbanArcade.mins)};
            var urbanInclusive = ${JSON.stringify(urbanArcade.inclusive)};
            var ruralLimits = ${JSON.stringify(ruralArcade.limits)};
            var ruralMins = ${JSON.stringify(ruralArcade.mins)};
            var ruralInclusive = ${JSON.stringify(ruralArcade.inclusive)};

            if (zona == 3) {${buildDensityZoneClassExpression("ruralLimits", "ruralMins", "ruralInclusive", '"R"')}
            }
            var prefijo = IIF(zona == 2, "P", "C");
            ${buildDensityZoneClassExpression("urbanLimits", "urbanMins", "urbanInclusive", "prefijo")}
        `,
...
function getDensityColorByZone(value, tzn = 1, departmentCode = null) {
    const tipo = Number(tzn) === 3 ? "rural" : "urbano";
    const breaks = departmentCode ? getDensityBreaksForDepartment(departmentCode) : nationalDensityBreaks;
    const palette = tipo === "rural" ? ruralDensityColors : urbanCenterDensityColors;
    const groupBreaks = breaks[tipo] || [];
    const resolvedIndex = resolveDensityBreakIndex(groupBreaks, value);
    return resolveDensityPaletteColor(
        palette,
        resolvedIndex,
        groupBreaks.length,
...
function getDensityLegendItems(departmentCode = null) {
    const breaks = departmentCode ? getDensityBreaksForDepartment(departmentCode) : nationalDensityBreaks;
    const items = [];
    const pushItems = (group, prefix, palette, groupTitle, zoneFilter, alignTail = false) => {
        ...
        groupBreaks.forEach((item, index) => {
            const entry = {
                label: item.label,
                color: resolveDensityPaletteColor(palette, index, groupBreaks.length, { alignTail }),
                code: `${prefix}${index + 1}`,
                group: groupTitle,
                max: item.max,
    ...
    pushItems("rural", "R", ruralDensityColors, "Rural disperso", 3, false);
    return items;
}

function buildDensityLegendItemWhere(item) {
...
function getColorByDensity(value) {
    for (const cls of densityClassBreaks) {
...
const compositionOrder = ["nm", "nf", "jm", "jf", "am", "af", "amm", "amf"];

const nationalCompositionColors = ["#FDE0DD", "#FCAE91", "#FB6A4A", "#EF3B2C", "#CB181D", "#A50F15", "#67000D"];
const urbanMaleCompositionColors = ["#D1FF73", "#9DDB96", "#68B8B9", "#3494DC", "#0070FF"];
const ruralMaleCompositionColors = ["#E9FFBE", "#B9FFB7", "#AFFFDF", "#A8EBFF", "#A1AEFF"];
const urbanFemaleCompositionColors = ["#FFD37F", "#EFA3BF", "#DF73FF", "#C43AF2", "#A900E6"];
const ruralFemaleCompositionColors = ["#F5A27A", "#ED848F", "#E68EBD", "#F24BDE", "#C500FF"];
const lowDensityCompositionDepartments = ["18", "27", "63", "66", "81", "85", "86", "88", "91", "94", "95", "97", "99"];

const nationalCompositionBreaks = {
...
const municipalCompositionBreaks = {
    baja: {
        urbano: {
...
function formatCompositionNumber(value) {
...
function buildCompositionLegendItems(values = [], colors = [], group = "") {
    let previous = 0;
    return values.map((max, index) => {
        const label = index === 0
            ? `<= ${formatCompositionNumber(max)} personas`
            : `${formatCompositionNumber(previous + 1)} - ${formatCompositionNumber(max)} personas`;
        const item = {
            label,
...
function getCompositionDensityGroup(departmentCode = "") {
    const code = String(departmentCode || "").padStart(2, "0").slice(0, 2);
    return lowDensityCompositionDepartments.includes(code) ? "baja" : "alta";
}

function getCompositionLegendItems(field = "nm", options = {}) {
    const activeField = compositionOrder.includes(field) ? field : "nm";
    if (options.national) {
        return buildCompositionLegendItems(nationalCompositionBreaks[activeField] || [], nationalCompositionColors, "Nacional");
    }

    const densityGroup = getCompositionDensityGroup(options.departmentCode);
    const gender = activeField.endsWith("f") ? "femenino" : "masculino";
    const urbanoColors = gender === "femenino" ? urbanFemaleCompositionColors : urbanMaleCompositionColors;
    const ruralColors = gender === "femenino" ? ruralFemaleCompositionColors : ruralMaleCompositionColors;
    return [
        ...buildCompositionLegendItems(municipalCompositionBreaks[densityGroup].urbano[activeField] || [], urbanoColors, "Cabecera y centros poblados"),
        ...buildCompositionLegendItems(municipalCompositionBreaks[densityGroup].rural[activeField] || [], ruralColors, "Rural disperso")
    ];
}

function buildCompositionRenderer(field = "nm", options = {}) {
    const activeField = compositionOrder.includes(field) ? field : "nm";
    const legendItems = getCompositionLegendItems(activeField, options);
    ...
    const nationalLimits = JSON.stringify(nationalCompositionBreaks[activeField] || []);
    const densityGroup = getCompositionDensityGroup(options.departmentCode);
    const urbanoLimits = JSON.stringify(municipalCompositionBreaks[densityGroup].urbano[activeField] || []);
    const ruralLimits = JSON.stringify(municipalCompositionBreaks[densityGroup].rural[activeField] || []);

    return {
        type: "unique-value",
        valueExpressionTitle: compositionColors[activeField]?.label || activeField,
        valueExpression: `
            var value = Number($feature.${activeField});
            var zone = Number($feature.tzn);
            var isNational = ${options.national ? "true" : "false"};
            var limits = IIf(isNational, ${nationalLimits}, IIf(zone == 3, ${ruralLimits}, ${urbanoLimits}));
            var prefix = IIf(isNational, "Nacional", IIf(zone == 3, "Rural disperso", "Cabecera y centros poblados"));
            for (var i = 0; i < Count(limits); i++) {
                if (value <= limits[i]) return prefix + Text(i + 1);
            }
            return prefix + Text(Count(limits));
        `,
        uniqueValueInfos: infos,
...
function getSelfRecognitionLegendItems() {
    return selfRecognitionBreaks.map((item, index) => ({
        label: item.label,
        color: item.color,
        code: `pobtet_${index + 1}`,
        min: index === 0 ? null : selfRecognitionBreaks[index - 1].max,
        max: item.max,
        field: "pobtet"
    }));
}
...
function buildSelfRecognitionRenderer() {
    return {
        type: "class-breaks",
        field: "pobtet",
        classBreakInfos: getSelfRecognitionLegendItems().map((item, index) => ({
            minValue: index === 0 ? 0 : item.min,
            maxValue: item.max,
            label: item.label,
            symbol: {
...
if (typeof window !== "undefined") {
    Object.assign(window, {
        buildPopulationDensityRenderer,
        getDensityLegendItems,
        buildDensityLegendItemWhere,
        getDensityColorByZone,
        densityLabelToTzn,
        tznLabels,
        ...
        buildCompositionRenderer,
        getCompositionLegendItems,
        buildSelfRecognitionRenderer,
        getSelfRecognitionLegendItems,
